...
if (! function_exists('get_video_url')) {
    function get_video_url($file_name = '') {
        ...
        if(File::exists('public/storage/videos/'.$file_name)){
            return asset('storage/videos/'.$file_name);
        }else{
            return asset('storage/videos/default.mp4');
        }
    }
}
...
if ( ! function_exists('get_phrase'))
{
    function get_phrase($phrase = '', $value_replace = array()) {
        ...
        if (array_key_exists($phrase, $translations[$active_language])) {
            ...
            $dbCheck = DB::table('languages')->where('name', $active_language)->where('phrase', $phrase)->first();
            if ($dbCheck) {
                $tValue = $dbCheck->translated;
                $translations[$active_language][$phrase] = $tValue;
                Cache::put("translations_{$active_language}", $translations[$active_language], 3600);
            } else {
                $tValue = $phrase;
                ...
                if($all_language->count() > 0){
                    foreach($all_language as $language){
                        if(DB::table('languages')->where('name', $language->name)->where('phrase', $phrase)->count() == 0){
                ...
                Cache::forget("translations_{$active_language}");
                $translations[$active_language][$phrase] = $tValue;
            }
        }
...
if (! function_exists('get_product_image')) {
    function get_product_image($file_name = "", $foldername = "") {
        ...
        $defaultPath = public_path("storage/marketplace/$foldername/default/default.png");
        ...
        if(!empty($file_name) && file_exists($imagePath)){
            ...
            return asset("storage/marketplace/$foldername/default/default.png");
        }
    }
}
...
if (! function_exists('get_page_cover_photo')) {
    function get_page_cover_photo($file_name = "", $foldername = "coverphoto") {
        ...
        // 4. Fallback
        return asset('storage/pages/default.png');
    }
