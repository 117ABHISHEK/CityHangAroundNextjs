...
{
    private function getSingleUserViewData($id): array
    {
        $user = User::find($id);
   ...
   public function view_profile_data($id){
    ...
    $posts =  Posts::where('user_id', $id)->where('publisher','post')->where('privacy','public')
    ->where('report_status',0)
    ->orderBy('post_id','DESC')->limit('10')->get();

    $page_data['posts'] = $posts;
    $page_data = array_merge($page_data, $this->getSingleUserViewData($id));
