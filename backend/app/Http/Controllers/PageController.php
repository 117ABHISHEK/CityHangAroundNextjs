...
use App\Services\BusinessFeaturedProductService;
...
class PageController extends Controller
{
    ...
    public function category(Request $request, string $category_slug)
    {
        ...
        if (!auth()->check()) {
            ...
            $htmlCacheKey = "category_page_html_v5_{$category->id}_{$queryHash}";
        ...
        $mypages = Cache::remember($cacheKey, now()->addMinutes(10), function () use ($category, $filter_city, $filter_area, $filter_sort_by) {
            ...
            foreach ($results as $page) {
                $page->likes_count = $likeCounts[$page->id] ?? 0;
            }
        ...
        if (!auth()->check()) {
            $rendered = view('frontend.page_index', $page_data)->render();
    ...
    public function city(Request $request, $city_slug)
    {
        ...
        // FULL-PAGE HTML CACHE for guests — checked FIRST to bypass all database queries!
        if (!auth()->check()) {
            ...
            $cachedHtml = Cache::get($htmlCacheKey);
            \Illuminate\Support\Facades\Log::error("Full page HTML Cache check for city {$city_slug}: " . ($cachedHtml ? "HIT (length " . strlen($cachedHtml) . ")" : "MISS") . " | Key: {$htmlCacheKey}");
            if ($cachedHtml) {
                return response($cachedHtml)->header('Content-Type', 'text/html; charset=UTF-8');
            }
        } else {
            \Illuminate\Support\Facades\Log::error("Full page HTML Cache bypassed: auth user logged in: " . auth()->id());
        }
        ...
        if (!is_null($city)) {
             ...
             * OPTIMIZED: pluck + whereIn is ~100x faster than whereExists subquery.
             */
            $page_data['all_categories'] = Cache::remember("cityguide_parent_categories_city_v2_{$city->id}", 1800, function () use ($city) {
            ...
            // OPTIMIZED: Fetch and cache categories for this city, fetching only top 4 pages per category instead of loading thousands.
            $categories = $page_data['all_categories'];

            // We can cache the final pagesByCategory to speed up subsequent loads
            $pagesByCategory = Cache::remember("city_pages_by_category_sliced_{$city->id}", 600, function () use ($city, $categories) {
                // 1. Fetch raw IDs and order them in database (ultra-fast query using pages_covering_city_page_idx index scan)
                ...
                // Cache a simple parent category lookup mapping to avoid database join
                ...
                // Slice top 4 page IDs per parent category in PHP (microsecond speed)
                $slicedPageMappings = [];
                $categoryPageCounts = [];
                ...
                foreach ($slicedPageMappings as $mapping) {
                    ...
                    if (isset($pagesLookup[$pageId]) && isset($grouped[$parentCatId])) {
                        ...
                        if (!collect($grouped[$parentCatId])->contains('id', $page->id)) {
                            $grouped[$parentCatId][] = $lightPage;
                        }
                    }
                }
            ...
            $page_data['pagesByCategory'] = $pagesByCategory;

            // recentBusinesses: take first 6 from flat unique list
            ...
            if (auth()->check()) {
                ...
                // Cache the global public timeline feed for guests to make it instant
                $posts = Cache::remember("timeline_feed_guest_posts_v4", 300, function () {
                    return \App\Models\Posts::where('posts.status', 'active')
                        ->where('posts.report_status', '0')
            ...
            // Trending Listings for right sidebar (city-filtered) - reuse preloaded flat city pages
            $page_data['recentBusinesses'] = $allCityPagesFlat;

            // Trending Events for right sidebar (city-filtered) - CACHED
            $page_data['recentEvents'] = Cache::remember("recent_events_city_{$city->id}", 300, function () use ($city) {
                return \App\Models\Event::with(['city', 'area', 'categories'])
            ...
            // FULL-PAGE HTML CACHE for guests — store the rendered HTML
            if (!auth()->check()) {
                $queryHash = md5(http_build_query([
                    'area'           => $request->input('area', '0'),
                    'category'       => $request->input('category', '0'),
                ...
                $htmlCacheKey = "city_page_html_v4_{$city->id}_{$queryHash}";
                $rendered = view('frontend.page_index_city', $page_data)->render();
                Cache::put($htmlCacheKey, $rendered, 300); // 5 minutes
                \Illuminate\Support\Facades\Log::error("Full page HTML Cache WRITTEN for city {$city_slug} | Key: {$htmlCacheKey} | Length: " . strlen($rendered));
                return response($rendered)->header('Content-Type', 'text/html; charset=UTF-8');
            }
    ...
    public function categoryByCityArea(Request $request, $city_slug, $category_slug, $area_slug)
    {
        ...
        if (!auth()->check()) {
            $queryHash = md5(http_build_query($request->all()));
            $htmlCacheKey = "category_city_area_html_v5_{$category->id}_{$city->id}_{$area->id}_{$queryHash}";
            $cachedHtml = Cache::get($htmlCacheKey);
            if ($cachedHtml) {
                return response($cachedHtml)->header('Content-Type', 'text/html; charset=UTF-8');
            }
        ...
        $mypages = Cache::remember($cacheKey, now()->addMinutes(10), function () use ($category, $city, $filter_area, $filter_category, $filter_subcategory, $filter_search, $filter_sort_by, $perPage) {
            $query = Page::with([
                ...
                ->where('item_status', 2)
                ->where('city_id', $city->id)
                ->where('area_id', $filter_area !== '0' && !empty($filter_area) ? $filter_area : $city->areas()->where('area_slug', request()->route('area_slug'))->select('id')->first()?->id ?? 0)
                ->whereHas('categories', function ($q) use ($category) {
                    $q->where('pagecategories.id', $category->id)
                        ->orWhere('pagecategories.category_parent_id', $category->id);
                });
        ...
        // Step 6: Return
        if (!auth()->check()) {
            $rendered = view('frontend.page_index_area_city_category', compact(
                'city',
                'area',
                'category',
                'parent_categories',
    ...
    public function categoryByCity(Request $request, $category_slug, $city_slug)
    {
        ...
        if (!auth()->check()) {
            $queryHash = md5(http_build_query($request->all()));
            $htmlCacheKey = "category_city_html_v6_{$category->id}_{$city->id}_{$queryHash}";
            $cachedHtml = Cache::get($htmlCacheKey);
            if ($cachedHtml) {
                return response($cachedHtml)->header('Content-Type', 'text/html; charset=UTF-8');
            }
        ...
        $parentcategories = Cache::remember("parent_categories_city_{$city->id}_{$category->category_parent_id}", 3600, function () use ($category, $city) {
        ...
        SEOMeta::setDescription(app(CategoryCitySeoService::class)->description($category, $city));
        ...
        $cacheKey = "category_city_pages_" . md5(json_encode([
            $category->id,
            $city->id,
            $filter_area,
            $filter_category,
        ...
        $pages = Cache::remember($cacheKey, now()->addMinutes(10), function () use ($category, $city, $filter_area, $filter_category, $filter_subcategory, $filter_search, $filter_sort_by) {
            ...
            $results = $pagesQuery->paginate(50);
        ...
        if (!auth()->check()) {
            $rendered = view('frontend.page_filter_index', $page_data)->render();
            Cache::put($htmlCacheKey, $rendered, 300); // 5 minutes
            return response($rendered)->header('Content-Type', 'text/html; charset=UTF-8');
        }
    ...
    public function edit($id)
    {
        ...
        $page_data['all_service_city'] = DB::table('cities')
            ...
            ->where(function ($query) use ($page_data) {
                ...
                foreach ($cityIds as $cityId) {
                    $query->orWhereRaw("? = ANY(string_to_array(state_id, ',')::bigint[])", [$cityId]);
                }
            })
            ->get();
        ...
        $page_data['all_service_areas'] = DB::table('areas')
            ...
            ->where(function ($query) use ($page_data) {
                ...
                foreach ($cityIds as $cityId) {
                    $query->orWhereRaw("? = ANY(string_to_array(city_id, ',')::bigint[])", [$cityId]);
                }
            })
            ->get();
        ...
        $page_data['all_countries'] = DB::table('countries')
            ...
            ->where(function ($query) use ($page_data) {
                ...
                foreach ($cityIds as $cityId) {
                    $query->orWhereRaw("? = ANY(string_to_array(id, ',')::bigint[])", [$cityId]);
                }
            })
            ->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        Cache::forget("single_page_html_v3_{$page->item_slug}");
    ...
    {
        // Get all cities (Cached via helper)
        $page_data['all_cities'] = CityHelper::getActiveCities();

        // Get page
        $page = Cache::remember("page_$item_slug", 3600, function () use ($item_slug) {
            return Page::with(['city', 'area', 'categories', 'openingHours'])
                ->withCount('products')
        ...
        if (!$page && preg_match('/^(.+)-(\d+)$/', $item_slug, $matches)) {
            ...
            $base_page = Page::with(['city', 'area', 'categories', 'openingHours'])
                ->withCount('products')
        ...
        if (auth()->user()) {
            ...
            $page_view_data = $page->view ? json_decode($page->view, true) : [];
            if (!in_array(auth()->user()->id, $page_view_data)) {
                $page_view_data[] = auth()->user()->id;
                $page->view = json_encode($page_view_data);
                $page->save();
                Cache::forget("page_$item_slug");
            }
        }
        ...
        if (!$page->relationLoaded('category') || !$page->category) {
        ...
        if (!$page->relationLoaded('area') || !$page->area) {
            $areaObj = DB::table('areas')->where('area_slug', $area_slug)->first();
            if (!$areaObj) {
                $areaObj = (object)[
                    'id' => 0,
                    'area_name' => ucwords(str_replace('-', ' ', $area_slug)),
                    'area_slug' => $area_slug
                ];
            }
            $page->setRelation('area', $areaObj);
        }
        $parentcategories = Cache::remember("parent_categories_v2_{$category->category_parent_id}", 3600, function () use ($category) {
            return Pagecategory::where('id', $category->category_parent_id)->get();
        });
        ...
        $categoryNames = $page->categories->pluck('category_name')->implode(', ');
        ...
        $reviews = Cache::remember("page_reviews_6_{$page->id}", 600, function() use ($page) {
            return Review::where('marketplace_id', $page->id)
                ->with('user')
                ->where('type', 'pages')
                ->latest()
                ->take(6)
        ...
        $page_data['reviews'] = $reviews->take(5); // First 5 reviews
        $page_data['has_more_reviews'] = $reviews->count() > 5;

        $ratingStats = Cache::remember("page_rating_stats_{$page->id}", 600, function() use ($page) {
            return Review::where('marketplace_id', $page->id)
                ->where('type', 'pages')
                ->selectRaw('COALESCE(AVG(rating), 4.0) as avg_rating, COUNT(*) as review_count')
                ->first();
        });

        $page_data['rating'] = round($ratingStats->avg_rating, 1);
        $page_data['review_count'] = $ratingStats->review_count;
        ...
        $page_data['featuredProducts'] = app(BusinessFeaturedProductService::class)
            ->forBusiness($page);

        // FULL-PAGE HTML CACHE for guests — eliminates repeated Blade rendering (~6s → <200ms)
        if (!auth()->check()) {
            $htmlCacheKey = "single_page_html_v3_{$item_slug}";
            $cachedHtml = Cache::get($htmlCacheKey);
            if ($cachedHtml) {
                return response($cachedHtml)->header('Content-Type', 'text/html; charset=UTF-8');
            }
            $rendered = view('frontend.index', $page_data)->render();
            Cache::put($htmlCacheKey, $rendered, 300); // 5 minutes
            return response($rendered)->header('Content-Type', 'text/html; charset=UTF-8');
        }
