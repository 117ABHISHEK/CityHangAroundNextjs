...
use App\Models\{Blogcategory,Brand,Category,Currency,Pagecategory,Eventcategory,User,Page,Page_like,Blog,FileUploader, Payment_gateway,Ticket,TicketComment,PageMedia,IncompleteListing,ManageApproval,ClaimListing};
    ...
    public function searchclaimListings(Request $request)
{
    ...
    $data = $claims->map(function ($claim) {
        ...
        return [
            ...
            'current_owner' => optional($page?->user)->name ?? 'N/A',
            'ownership_proof' => $claim->ownership_proof,
            'is_approved' => $claim->is_approved,
            'claimant_name' => optional($claim->user)->name ?? 'Unknown',
            'claimant_email' => optional($claim->user)->email ?? 'Unknown',
    ...
    public function claimListings(Request $request)
    {
        $query = DB::table('claim_listings')
            ->select(
                'claim_listings.*',
                'claimant_users.name as claimant_name',
                'claimant_users.email as claimant_email',
                'pages.title',
                'pages.item_slug',
                'pages.city_id',
                'pages.area_id',
                'cities.city_slug',
                'areas.area_slug',
                'page_owner.name as current_owner',
                'claim_listings.created_at as submitted_at',
            )
            ->join('users as claimant_users', 'claim_listings.user_id', '=', 'claimant_users.id')
            ->join('pages', 'claim_listings.page_id', '=', 'pages.id')
            ->leftJoin('cities', 'cities.id', '=', 'pages.city_id')
            ->leftJoin('areas', 'areas.id', '=', 'pages.area_id')
            ->leftJoin('users as page_owner', 'page_owner.id', '=', 'pages.user_id');
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                ...
                ->orWhere('claimant_users.name', 'like', "%{$search}%")
                ->orWhere('claimant_users.email', 'like', "%{$search}%");
            });
        }
    ...
    public function updateClaimStatus(Request $request)
{
    ...
    // Notify claimant of decision
    $claimantId = $claim->user_id;
    $pageId = $claim->page_id;
    if ($claimantId && $pageId) {
        $message = $request->status === 'Y'
            ? 'Your claim for this listing has been approved!'
            : 'Your claim for this listing has been rejected.';
        DB::table('notifications')->insert([
            'sender_user_id' => auth()->id(),
            'reciver_user_id' => $claimantId,
            'type' => 'claim_listing_decision',
            'page_id' => $pageId,
            'status' => '0',
            'view' => '0',
            'created_at' => now(),
...
public function checkIfCategoryExists(Request $request)
{
    ...
    ->leftJoin('pages', function ($join) {
        $join->on('pages.city_id', '=', 'cities.id')
            ->whereRaw("pagecategories.id = ANY(string_to_array(pages.category_id, ',')::bigint[])");
    })
    ->leftJoin('cities', 'cities.id', '=', 'pages.city_id')
    ->leftJoin('areas', 'areas.id', '=', 'pages.area_id')
    ->join('users', 'users.id', '=', 'pagecategories.category_createdby')
    ...
    $query = DB::table('categories')
        ...
        ->leftJoin(DB::raw('(
            SELECT cp.product_category_id, 
                   SUM(CASE WHEN m.view IS NOT NULL AND m.view != \'\' THEN jsonb_array_length(m.view::jsonb) ELSE 0 END) as view_count
...
public function view_user_suggest_product_category()
{
    ...
    foreach ($categories as $category) {
        ...
        $viewCount = DB::table('category_product')
            ...
            ->sum(DB::raw('jsonb_array_length(marketplaces.view::jsonb)'));
...
public function productAjax(Request $request)
{
    ...
    function product_edit(Request $request){
        ...
        $selectedCategoryIds = collect(explode(',', (string) $product->category))
            ->map(fn ($value) => (int) trim($value))
            ->filter()
            ->values();

        $page_data['product'] = $product;
        $page_data['productimage'] = Media_files::where('product_id', $product->id)->get();
        $page_data['listing'] = collect();
        if (!empty($product->page_id)) {
            $page_data['listing'] = DB::table('pages')
                ->select('id', 'title')
                ->where('id', $product->page_id)
                ->get();
        }

        $page_data['product_categories'] = Category::query()
            ->select('id', 'product_category_name')
            ->whereIn('id', $selectedCategoryIds)
            ->orderBy('product_category_name')
            ->get();

        $page_data['brands'] = Brand::query()
            ->select('id', 'name')
            ->orderBy('name')
            ->get();

        $page_data['currencies'] = Currency::query()
            ->select('id', 'name')
            ->orderBy('name')
            ->get();
    ...
    public function productListingSearch(Request $request)
    {
...
public function toggleVerified(Request $request)
{
    ...
    function product_updated(Request $request,$id){
        ...
        ->distinct('pages.id')->orderBy('pages.id','DESC')->first();
        ...
        if ($product_id) {
          ...
          $slug_count=DB::table('marketplaces')->select('marketplaces.id')
            ...
            flash()->addSuccess(get_phrase('Marketplace Product Updated Successfully'));
    ...
    function product_created(Request $request){
        ...
        if ($product_id) {
            ...
            if(is_array($request->multiple_files) && $request->multiple_files[0] != null){
             ...
             flash()->addSuccess(get_phrase('Marketplace Product Added Successfully'));
            return redirect(route('admin.product'));
        }
...
public function approveAll(Request $request)
{
    ...
    function product_create(){
        $page_data['listing'] = collect();
        $page_data['product_categories'] = collect();
        $page_data['brands'] = Brand::query()
            ->select('id', 'name')
            ->orderBy('name')
            ->get();
    ...
    function groups(){
        ...
        ->distinct('groups.id')->orderBy('groups.id','DESC')->paginate('10');
    ...
    function blogs(){
        ...
        ->distinct('blogs.id')->orderBy('blogs.id','DESC')->get();
  ...
  function blog_create(){
    ...
    function blog_edit($id=""){
        ...
        $parentCategories = DB::table('pagecategories')
            ->select('id', 'category_name')
            ->whereNull('category_parent_id')
            ->get();

        $selectedCategoryIds = collect(explode(',', (string) $page_data['blog_details']->category_id))
            ->map(fn ($value) => (int) trim($value))
            ->filter()
            ->values();

        $page_data['parent'] = $parentCategories;
        $page_data['printable_categories'] = $parentCategories;

        $page_data['blog_categories'] = \App\Models\Blogcategory::query()
            ->select('id', 'category_name')
            ->whereIn('id', $selectedCategoryIds)
        ...
        $page_data['all_cities'] = DB::table('cities')
            ->select('id', 'city_name')
            ->where('state_id', $page_data['blog_details']->state_id)
            ->get();

        $page_data['all_areas'] = DB::table('areas')
            ->select('id', 'area_name')
            ->where('city_id', $page_data['blog_details']->city_id)
            ->get();

        $page_data['listing'] = collect();
        if (!empty($page_data['blog_details']->list_id)) {
            $page_data['listing'] = DB::table('pages')
                ->select('id', 'title')
                ->where('id', $page_data['blog_details']->list_id)
                ->get();
        }
    ...
    public function blogListingSearch(Request $request)
    {
        $search = trim((string) $request->input('q', ''));
