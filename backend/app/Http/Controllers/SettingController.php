...
class SettingController extends Controller
{
    ...
    public function term_view(Request $request)
    {
        $page_data['term'] = Setting::where('type', 'term')->value('description');
        $page_data['noimage'] = $request->has('noimage');
    ...
    public function system_settings_logo_save(Request $request)
    {
        if ($request->hasFile('dark_logo')) {
            ...
            // if ($done) {
                $dark_file_name =FileUploader::upload($request->dark_logo,'public/storage/logo/dark/'.$request->dark_logo);
                $done = Setting::where('type', 'system_dark_logo')->update(['description' => $dark_file_name]);
            // }
        }
        ...
        if ($request->hasFile('light_logo')) {
            ...
            // if ($done) {
                 $light_file_name=FileUploader::upload($request->light_logo,'public/storage/logo/light/'.$request->light_logo);
                 $done = Setting::where('type', 'system_light_logo')->update(['description' => $light_file_name]);
            // }
        }
        ...
        if ($request->hasFile('favicon')) {
            ...
            // if ($done) {
                $favicon_file_name =FileUploader::upload($request->favicon,'public/storage/logo/favicon/'.$request->favicon);
                $done = Setting::where('type', 'system_fav_icon')->update(['description' => $favicon_file_name]);
            // }
        }
