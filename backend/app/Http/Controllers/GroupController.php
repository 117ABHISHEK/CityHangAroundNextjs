...
class GroupController extends Controller
{
    ...
    public function groups(Request $request)
    {
        ...
        $page_data["filter_city"] = empty($request->city) ? null : $request->city;
        $page_data["filter_area"] = empty($request->area) ? "0" : $request->area;
        $page_data["filter_sort_by"] = empty($request->filter_sort_by) ? "newest" : $request->filter_sort_by;
    ...
    public function category_group(Request $request, string $category_slug)
    {
        ...
        if ($category) {
            ...
            $paid_items_query = DB::table("groups")
                ...
                ->orderByDesc("groups.id") // ✅ Must be first to match DISTINCT ON
                ->orderByDesc("groups.item_featured") // ✅ Then featured
                ->distinct("groups.id");
    ...
    public function jsonGetAreasByCityforitem(int $city_id)
    {
        $areas = DB::table("areas")
            ...
            ->distinct("groups.id")
            ->orderByDesc("groups.id") // ✅ Must match DISTINCT ON first
            ->orderByDesc("groups.item_featured") // ✅ Then featured
            ->where("groups.group_status", 2)
    ...
    public function groupcategorycity(
        ...
        $page_data["all_categories"] = Cache::remember('active_community_parent_categories', 3600, function () use ($city, $category) {
            return DB::table("groupcategories")
                ->select("groupcategories.*")
                ->join(
                    "group_category",
        ...
        $page_data["categories"] = $page_data["all_categories"];

        if (!is_null($category) && !is_null($city)) {
            $filter_city = $city->id;
            $page_data['filter_city'] = $city->id;
            $page_data['filter_area'] = '0';
    ...
    public function categorycityarea(
        ...
        $page_data["city"] = $city;
        $page_data["category"] = $category;
        $page_data['filter_city'] = $city ? $city->id : null;
        $page_data['filter_area'] = $area ? $area->id : '0';
        $page_data["all_cities"] = CityHelper::getActiveCities();
    ...
    public function single_group(
        ...
        // Fetch all cities (for dropdowns or filtering) - CACHED
        $page_data["all_cities"] = Cache::remember('active_cities_dropdown', 3600, function() {
            return DB::table("cities")
                ->select("cities.*")
        ...
        // If category_slug is numeric, it is an ID-based details route (e.g. group/view/details/76)
        if (is_numeric($category_slug)) {
            $group = DB::table("groups")
                ->where("id", $category_slug)
                ->first();
            if (!$group) {
                abort(404, "Group not found");
            }
            $item_slug = $group->group_slug;

            // Resolve category slug
            $item_category = DB::table('group_category')
                ->where('group_id', $group->id)
                ->first();
            if ($item_category) {
                $db_cat = DB::table('groupcategories')->where('id', $item_category->category_id)->first();
                $category_slug = $db_cat ? $db_cat->category_slug : 'social-group';
            } else {
                $category_slug = 'social-group';
            }

            // Resolve city slug
            if ($group->city_id) {
                $db_city = DB::table('cities')->where('id', $group->city_id)->first();
                if ($db_city) {
                    $city_slug = $db_city->city_slug;
                }
            }

            // Resolve area slug
            if ($group->area_id) {
                $db_area = DB::table('areas')->where('id', $group->area_id)->first();
                if ($db_area) {
                    $area_slug = $db_area->area_slug;
                }
            }
        } else {
        ...
        // Get parent categories based on city & category parent - CACHED
        $parentcategories = collect();
        if ($city) {
            $parentcategories = Cache::remember("parent_categories_city_{$city->id}_parent_{$category->category_parent_id}", 600, function() use ($city, $category) {
                return DB::table("groupcategories")
                    ->select("groupcategories.*")
                    ->join(
                        "group_category",
        ...
        $page_data['all_categories'] = Cache::remember('all_group_categories', 3600, fn() => Groupcategory::all());
