...
class SitemapController extends Controller
{
    ...
    {
        $totalPages = DB::table('pages')->where('item_status', 2)->count();
        $limit = 40000;
...
public function sitemaplisting6()
{
    ...
    public function pagesSitemap($part = 1)
    {
        ...
        $urls = [];
        $limit = 40000; // URLs per sitemap file
        $offset = ($part - 1) * $limit;

        // Optimized query with pagination
        $pages = DB::select(DB::raw('select DISTINCT pages.id,pages.item_slug,cities.city_slug,areas.area_slug,pagecategories.category_slug,pages.updated_at from pages
        inner join cities on cities.id=pages.city_id
        inner JOIN areas on areas.id=pages.area_id
        inner join(select max(id) as max,max(category_id) as category_id,page_id
            from page_category  group by page_id) t1
            on t1.page_id=pages.id
            inner JOIN pagecategories on pagecategories.id=t1.category_id
        where pages.item_status=2 and pages.id>0
        ORDER BY pages.id
    ...
    public function blogsSitemap()
    {
        ...
        $blogs = Blog::where('blog_status', 2)
            ->with(['categories', 'city', 'area'])
            ->select('id', 'title', 'updated_at', 'blog_slug', 'city_id', 'area_id')
            ->get();
    ...
    public function groupsSitemap()
    {
        ...
        foreach($groups as $group) {
            $urls[] = [
                'loc' => route('single.group', ['id' => $group->id]),
