...
$page_data['events'] = $events;
    ...
    public function store(Request $request)
    {
        ...
        if ($activeSubscription && $activeSubscription->subscription && Str::contains($activeSubscription->subscription->offered_services, 'event')) {
            ...
            if ($cityDays > 0) $event->priority_until_city = $subscriptionStart->copy()->addDays($cityDays);
            if ($areaDays > 0) $event->priority_until_area = $subscriptionStart->copy()->addDays($areaDays);
            if ($priorityEnd->isFuture()) $event->item_featured = 1;
        }
        $done = $event->save();
        if ($done) {
    ...
    public function single_event($city_slug,$area_slug,$category_slug,$event_slug)
    {
        ...
        $event_id=DB::table('events')->select('id')->where('event_slug',$event_slug)
        ->get();
        if($event_id){
            $id=$event_id[0]->id;
        }
        ...
        $events = Event::findOrFail($id); // or whatever method you're using

        if (auth()->user()){
            app(UserActivityService::class)->log(auth()->user()->id, 'view', 'event', $events->id,$events->id);
        }

            //print_r($pages);exit;

            if ($events) {
                $events_view_data = $events->view ? json_decode($events->view, true) : [];

                if (auth()->user() && !in_array(auth()->user()->id, $events_view_data)) {
                    $events_view_data[] = auth()->user()->id;
                    $events->view = json_encode($events_view_data);
                }
       ...
       ->orderBy('category_name')->get();
        ...
        foreach ($events as $event) {
            ...
            array_push($popularrate, ['id' => $event->id, 'popular' => $total, 'banner' => $event->banner, 'event_date' => $event->event_date, 'title' => $event->title, 'post_user' => $event->getUser->name, 'user_id' => $event->getUser->id, 'photo' => $event->getUser->photo, 'interested_users_id' => $event->interested_users_id,'city_id' => $event->city_id,'area_id' => $event->area_id,'event_slug' => $event->event_slug
        ]);
        }
    ...
    public function eventcategoryByCityArea(Request $request,$city_slug,$category_slug,$area_slug){
        ...
        $page_data['categories']= DB::table('eventcategories')->select('eventcategories.*')
        ->join('event_category','event_category.category_id','=','eventcategories.id')
        ->join('events','event_category.event_id','events.id')
        ->distinct('eventcategories.id')
        ->where('events.event_status',2)
        ->orderBy('eventcategories.id','DESC')
       // ->Where('eventcategories.category_parent_id',$category->id)
...
return view('frontend.event_category_city_area_index', $page_data);
    ...
    public static function geteventbycategoryid($categoryid,$cityid){
       ...
       ->where(function ($query) use ($categoryid) {
        ...
        ->distinct('events.id')->limit(4)->get();
    ...
    public function eventcategory(Request $request, string $category_slug){
        ...
        $page_data['all_categories']= DB::table('eventcategories')->select('eventcategories.*')
        ->join('event_category','event_category.category_id','=','eventcategories.id')
        ->join('events','event_category.event_id','events.id')
        ->distinct('eventcategories.id')
        ->where('events.event_status',2)
        ->where('events.event_date', '>=', now())
        ->orderBy('eventcategories.id','DESC')
        ->where('eventcategories.category_parent_id',0)
            ->orWhereNull('eventcategories.category_parent_id')
        ->get();

        $page_data['all_event_cities']= DB::table('cities')->select('cities.*')
...
$paid_items_query = DB::table('events')->select(
    'events.id', 'events.user_id',
    'events.event_slug', 'events.title',
    'events.city_id', 'events.area_id', 'events.banner',
    'events.event_date', 'events.event_time',
    'events.location', 'events.created_at',
    'events.item_featured', // ✅ important
    'cities.city_slug', 'cities.city_name',
    'areas.area_slug', 'areas.area_name',
    'states.state_name',
...
// ✅ Sorting: Featured first, then created_at
$paid_items_query->orderByDesc('events.item_featured');
