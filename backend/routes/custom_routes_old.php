...
Route::get('/categories/{id}/edit-lead-price', [CategoryController::class, 'edit'])->name('categories.editLeadPrice');  // Edit lead price
Route::post('/categories/{id}/update-lead-price', [CategoryController::class, 'update'])->name('categories.updateLeadPrice');  // Update lead price
Route::post('/categories/{id}/delete-lead-price', [CategoryController::class, 'deleteLeadPrice'])->name('categories.deleteLeadPrice');  // Remove lead price (set to NULL)
...
Route::controller(MarketplaceController::class)->middleware('auth', 'verified', 'activity', 'prevent-back-history')->group(function () {
    Route::get('/deals', 'allproducts')->name('allproducts');
...
Route::delete('/incomplete-listings/{id}', 'deleteIncompleteListing')->name('incomplete.delete');
    ...
    Route::get('ajax/itemareas/{city_id}', 'jsonGetAreasByCityforitem')->name('json.itemareas');
...
Route::controller(SearchController::class)->middleware('auth', 'verified', 'activity', 'prevent-back-history')->group(function () {
    ...
    Route::get('/search-globally', 'search_globally')->name('search.globally');
});
...
Route::group(['prefix' => 'deals'], function() {
    ...
    Route::get('{city_slug}/{category_slug}/{item_slug}/{product_category_slug}/{product_slug}',[MarketplaceController::class, 'single_product'] )->name('single.product')
    ->where('city_slug', '[A-Za-z0-9-]+')
    ->where('category_slug', '[A-Za-z0-9-]+')
