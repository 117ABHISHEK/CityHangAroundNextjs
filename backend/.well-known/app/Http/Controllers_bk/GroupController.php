...
class GroupController extends Controller
{
    ...
    public function submitReport(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'proof_attachment' => 'nullable|mimes:jpg,jpeg,png,pdf,docx|max:2048',
        ...
        if (auth()->user()){
            app(UserActivityService::class)->log(auth()->user()->id, 'report', $request->type,$request->entity_id,$request->entity_id);
        }
    ...
    public function groups(){
        SEOMeta::setTitle('Join & Explore Local Groups – Connect with Like-Minded People | CityHangaround');
        ...
        SEOMeta::setDescription('Find and join local groups based on your interests in business, networking, hobbies, events, and more. Connect with people in your city and grow your community!');
        SEOMeta::addKeyword(['local groups', 'community groups', 'networking groups', 'hobby groups', 'business groups', 'social groups', 'CityHangaround groups', 'join groups online']);
    ...
    if(auth()->user()){
    $page_data['managegroups'] = Group::select('groups.*', 'groupcategories.category_name')
    ->join('group_category', 'groups.id', '=', 'group_category.group_id')
    ->join('groupcategories', 'group_category.category_id', '=', 'groupcategories.id')
    ...
    ->get();
        ...
        $page_data['all_printable_categories']=DB::table('groupcategories')->select('groupcategories.*')
        ...
        //print_r( $page_data['all_printable_categories']);exit;
    


        $page_data['view_path'] = 'frontend.groups.groups';
        return view('frontend.index_group', $page_data);
    }
    ...
    public function userpgroup(){
        ...
        ->where('groups.group_status',2)->orderBy('id','DESC')->get();
    ...
    public function storecategories(Request $request)
    {
        ...
        if (auth()->user()){
            app(UserActivityService::class)->log(auth()->user()->id, 'category_suggest', 'group_category', $category->id, $category->id);
         }
    ...
    public function category_group(Request $request, string $category_slug){
        ...
        $category = Groupcategory::where('category_slug', $category_slug)->first(); 
        $page_data['category']=$category;
        $page_data['all_cities'] = CityHelper::getActiveCities();

        $page_data['all_categories']= DB::table('groupcategories')->select('groupcategories.*')
        ->join('group_category','group_category.category_id','=','groupcategories.id')
        ...
        $page_data['all_group_cities']= DB::table('cities')->select('cities.*')
...
$paid_items_query = DB::table('groups')
    ...
    ->when(!empty($filter_city), fn($q) => $q->where('groups.city_id', $filter_city))
    ->when(!empty($filter_area), fn($q) => $q->where('groups.area_id', $filter_area))
...
return view('frontend.group_index', $page_data);
    ...
    public function groupcategorycity(Request $request,$category_slug,$city_slug){
        //echo "123";exit;
       ...
       // ->Where('groupcategories.category_parent_id',$category->id)
             ->where(function ($query) use ($category) {
                $query->where('group_category.category_id', $category->id)
...
return view('frontend.group_category_city_filter_index', $page_data);
    ...
    public function group_area(Request $request, string $city_slug, string $area_slug){
...
return view('frontend.group_city_area_index', $page_data);
    ...
    public function categorycityarea(Request $request,$city_slug,$category_slug,$area_slug){
...
return view('frontend.group_city_category_area_index', $page_data);
    ...
    public function group_city(Request $request,$city_slug){
        ...
        if(!is_null($city))
        {
            SEOMeta::setTitle($city->city_name.',' . 'near by top business listing, deals, offers');
            SEOMeta::setDescription('Get top business listing, deals, offers, local business list near -'.$city->city_name);
            ...
            $categories= DB::table('groupcategories')->select('groupcategories.*')
            ->join('group_category','group_category.category_id','=','groupcategories.id')
            ->join('groups','group_category.group_id','groups.id')
            ->distinct('groupcategories.id')
            ->where('groups.group_status',2)
            ->orderBy('groupcategories.id','asc')
            ->where('groups.city_id',$city->id)
            ->where('groupcategories.category_parent_id',0)->get();
    ...
    ->where(function ($query) use ($categoryid) {
        $query->where('group_category.category_id', $categoryid)
    ...
    public function single_group($category_slug, $item_slug, $city_slug = null, $area_slug = null)
{
    ...
    $group = DB::table('groups')->where('group_slug', $item_slug)->first();
    if (!$group) {
        abort(404, 'Group not found');
    }
    if (auth()->user()){
        app(UserActivityService::class)->log(auth()->user()->id, 'view', 'group', $group->id, $group->id);
     }
    // Get the group ID
    $id = $group->id;
    ...
    public function store(Request $request){
       ...
       $user = auth()->user();
        ...
        if ($activeSubscription && $activeSubscription->subscription && Str::contains($activeSubscription->subscription->offered_services, 'group')) {
            ...
            if ($cityDays > 0) $group->priority_until_city = $subscriptionStart->copy()->addDays($cityDays);
            if ($areaDays > 0) $group->priority_until_area = $subscriptionStart->copy()->addDays($areaDays);
            if ($priorityEnd->isFuture()) $group->item_featured = 1;
        }
        $done = $group->save();
        if($done){
            ...
            if($done){
                ...
                if (auth()->user()){
                    app(UserActivityService::class)->log(auth()->user()->id, 'group_listing', 'group', $group->id, $group->id);
              }
    ...
    public function join($id){
        ...
        $group_member->save();
         if (auth()->user()){
            app(UserActivityService::class)->log(auth()->user()->id, 'follow', 'group', $id,$id);
         }
    ...
    public function group_all_view(){
        ...
        //$page_data['joinedgroups'] = Group_member::where('user_id',auth()->user()->id)->where('is_accepted','1')->limit('6')->get();
        $page_data['joinedgroups'] =  DB::table('groups')->select('groups.*')
        ->join('group_members','group_members.group_id','groups.id')
        ->orderBy('id','DESC')->where('group_members.user_id',auth()->user()->id)->limit('6')->get();
        $page_data['groups'] = Group::orderBy('id','DESC')->limit('6')->get();
