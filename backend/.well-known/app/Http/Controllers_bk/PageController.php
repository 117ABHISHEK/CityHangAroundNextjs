...
public function pages(Request $request)
{
    ...
    $cacheKey = 'pages_' . md5(json_encode([
        $filter_city, $filter_area, $filter_sort_by, request('page', 1)
    ]));

    $paginated = Cache::remember($cacheKey, now()->addMinutes(10), function () use (
        $relations, $filter_city, $filter_area, $filter_sort_by, $perPage
    ) {
    ...
    public function area(Request $request, string $city_slug, string $area_slug)
{
    ...
    // All cities with listings
    $all_cities = Cache::remember('all_cities_with_listings', $cache_ttl, function () {
        return City::whereHas('pages', function ($q) {
            $q->where('item_status', 2);
        })->orderBy('city_name')->get();
    });

    // All categories related to this area
    $all_categories = Cache::remember("categories_city_{$city->id}_area_{$area->id}", $cache_ttl, function () use ($city, $area) {
        return Pagecategory::whereHas('pages', function ($query) use ($city, $area) {
    ...
    //     ->orderBy('id', 'DESC')
    //     ->paginate(50)
    //     ->appends(['filter_sort_by' => $filter_sort_by]);
    ...
    //     $query->whereHas('user.subscriptions', function ($q) use ($city) {
    //         $q->where('status', 'active')
    //           ->where('expires_at', '>=', now())
    //           ->whereHas('subscription', function ($subQuery) use ($city) {
    //               $subQuery->where('offered_services', 'like', '%listings%')
    //                        ->where(function ($cityQuery) use ($city) {
    //                            $cityQuery->whereNull('offered_cities') // allow all if NULL
    //                                      ->orWhere('offered_cities', 'like', "%{$city->id}%");
    //                        });
    //           });
    //     })
    //     ->orWhereDoesntHave('user.subscriptions'); // include pages without any subscription
    // })
    // ->orderBy($sort_column, $sort_direction)
    // ->orderBy('id', 'DESC')
    // ->paginate(50)
    ...
    public function category(Request $request, string $category_slug)
{
    ...
    // Cache all cities with listings
    $page_data['all_cities'] = Cache::remember('all_cities_with_listings', now()->addHours(6), function () {
        return City::whereHas('pages', function($query) {
            $query->where('item_status', 2);
        })->orderBy('city_name')->get();
    });

    // Cache cities for this specific category
    $page_data['all_category_cities'] = Cache::remember("category_cities_{$category->id}", now()->addHours(6), function () use ($category) {
        return City::whereHas('pages.pageCategories', function($query) use ($category) {
    ...
    // Cache all top-level categories
    $page_data['all_categories'] = Cache::remember('all_top_categories', now()->addHours(6), function () {
        return Pagecategory::where(function($query){
            $query->where('category_parent_id', 0)
                  ->orWhereNull('category_parent_id');
        })->whereHas('pages', function($q) {
            $q->where('item_status', 2);
        })->orderByDesc('id')->get();
    });
...
public function checkMatch(Request $request)
{
    ...
    public function category_old(Request $request, string $category_slug){
        ...
        ->where('pagecategories.category_parent_id',0)
    ...
    public static function getpagesbycategoryid($categoryid,$cityid){
        ...
        //     $query->where('page_category.category_id', $categoryid)
        //         ->orWhere('pagecategories.category_parent_id',$categoryid);
        // })
        // // ->where('page_category.category_id', $categoryid)
        // ->where('pages.city_id', $cityid)
        // ->distinct('pages.id')->limit(4)->get();

      $paid_items_query = DB::table('pages')
    ->select(
        'pages.id', 'pages.item_slug', 'pages.logo', 'pages.title',
        'cities.city_slug', 'areas.area_slug', 'cities.city_name',
        'areas.area_name', 'states.state_name'
    )
    ->join('cities', 'cities.id', '=', 'pages.city_id')
    ->join('areas', 'areas.id', '=', 'pages.area_id')
    ->join('states', 'states.id', '=', 'pages.state_id')
    ...
    public function categoryByCityArea(Request $request, $city_slug, $category_slug, $area_slug)
{
    ...
    $sidebarData = Cache::remember($cacheKey, 3600, function () use ($category, $city, $area) {
        // Cities with relevant pages and their areas
        $all_cities = City::with(['areas' => function($query) use ($city) {
                $query->where('city_id', $city->id);
            }])
            ->whereHas('pages.categories', function ($q) use ($category) {
        ...
        $all_categories = Pagecategory::whereHas('pages', function ($q) use ($pageIds) {
                $q->whereIn('pages.id', $pageIds);
            })
    ...
    // 4. Main Listings
    // $pagesQuery = Page::with(['city:id,city_name,city_slug', 'area:id,area_name,area_slug', 'state:id,state_name', 'categories:id,category_name,category_slug,category_parent_id'])
    //     ->where('item_status', 2)
    //     ->where('city_id', $city->id)
    //     ->where('area_id', $area->id)
    //     ->whereHas('categories', function ($q) use ($category) {
    ...
    $cacheKey = 'category_pages_' . md5(json_encode([
        $category->id, $city->id, $filter_area, $filter_category, $filter_subcategory, $filter_search, $filter_sort_by, $request->get('page', 1)
    ]));

    $mypages = Cache::remember($cacheKey, now()->addMinutes(10), function () use (
        $category, $city, $filter_area, $filter_category, $filter_subcategory, $filter_search, $filter_sort_by, $perPage
    ) {
        $query = Page::with([
...
return view('frontend.page_index_area_city_category', compact(
    'city', 'area', 'category', 'parent_categories',
    'all_cities', 'all_categories', 'mypages', 'filter_sort_by',
    'filter_area', 'filter_category', 'filter_subcategory', 'filter_search'
...
public function categoryByCity(Request $request, $category_slug, $city_slug)
{
    ...
    $page_data['all_cities'] = Cache::remember('all_cities_with_pages', 3600, function () use ($city) {
        return City::with(['areas' => function($query) use ($city) {
                $query->where('city_id', $city->id);
            }])
            ->select('cities.*')
            ->join('pages', 'pages.city_id', 'cities.id')
    ...
    $page_data['all_categories'] = Pagecategory::select('pagecategories.*')
        ->join('page_category', 'page_category.category_id', '=', 'pagecategories.id')
        ->join('pages', 'page_category.page_id', 'pages.id')
        ->distinct('pagecategories.id')
        ->where('pages.item_status', 2)
    ...
    // $pagesQuery = Page::with(['city', 'area', 'state', 'pagecategories'])
    //     ->withCount('likes')
    //     ->where('item_status', 2)
    //     ->where('city_id', $city->id)
    //     ->where(function ($query) use ($category) {
    //         $query->whereHas('pagecategories', function ($q) use ($category) {
    //             $q->where('pagecategories.id', $category->id)
...
public function createCategoryFromSelect2(Request $request)
{
    ...
    public function store(Request $request){
      ...
      $user = auth()->user();
        ...
        $count = DB::table('pages')->where('title', $request->name)
        ->where('address', $request->address)
        ->count();
        ...
        if ($activeSubscription && $activeSubscription->subscription && Str::contains($activeSubscription->subscription->offered_services, 'listings')) {
            ...
            if ($cityDays > 0) $page->priority_until_city = $subscriptionStart->copy()->addDays($cityDays);
            if ($areaDays > 0) $page->priority_until_area = $subscriptionStart->copy()->addDays($areaDays);
            if ($priorityEnd->isFuture()) $page->item_featured = 1;
        }
    ...
    public function single_page($city_slug, $area_slug, $category_slug, $item_slug)
{
    ...
    $page_data['posts'] = Cache::remember("posts_$id", 300, function () use ($id) {
        return Posts::select(
            'posts.*',
            'pages.id as page_id', 'pages.item_slug', 'pages.logo', 'pages.title',
            'pages.coverphoto', 'pages.user_id', 'pages.description',
            'pages.job', 'pages.location', 'pages.lifestyle',
            'cities.city_slug', 'areas.area_slug',
            'pagecategories.category_slug', 'pagecategories.category_name',
