...
class VideoController extends Controller
{
    public function videos()
    {
        ...
        $page_data['vidoes'] = Video::where('category', 'video')->where('privacy', 'public')->where('approve_status',2)->orderBy('id', 'DESC')->limit(5)->get();
...
public function store(Request $request)
{
    ...
    $fileName = time() . '.' . $originalVideo->getClientOriginalExtension();
    $originalPath = public_path('storage/videos/' . $fileName);
    ...
    //     ->setKiloBitrate(800) // Lower bitrate to reduce file size
    //     ->save('videos/compressed/' . $compressedFileName);
    ...
    $video->approve_status = $approve_status;
    $video->file = $fileName; // Save compressed video path
    ...
    if (auth()->user()){
        app(UserActivityService::class)->log(auth()->user()->id, 'video_post', 'video',$video->id,$video->id);
     }
    // Success Message
    Session::flash('success_message', 'Video/Shorts Created Successfully');
    ...
    public function video_delete()
    {
        ...
        if ($done) {
            ...
            if (auth()->user()){
                app(UserActivityService::class)->deleteByvideoActivityId($video->id);
            }
