...
class MainController extends Controller
{
    ...
    function timeline()
    {
        ...
        $activeCutoff = now()->timestamp - 86400;

        // NOTE:
        // This query is used as a derived table via fromSub().
        // Never use implicit select(*) with joins because MySQL requires UNIQUE column names
        // in derived tables; both `stories` and `users` have a `status` column.
        $storyWindow = Stories::query()
            ->select([
                'stories.story_id',
                'stories.user_id',
                'stories.created_at',
            ...
            ->where(function ($query) use ($activeCutoff) {
                $query->whereNotNull('stories.expires_at')
                    ->where('stories.expires_at', '>=', now()->timestamp)
                    ->orWhere(function ($fallback) use ($activeCutoff) {
                        $fallback->whereNull('stories.expires_at')
                            ->where('stories.created_at', '>=', $activeCutoff);
                    });
            })
        ...
        $latestStoryIds = DB::query()
            ->fromSub($storyWindow, 'visible_stories')
        ...
        $stories = DB::table('stories')
            ->join('users', 'stories.user_id', '=', 'users.id')
            ->joinSub($latestStoryIds, 'story_groups', function ($join) {
        ...
        $page_data['my_story'] = Stories::where('user_id', $this->user->id)
            ...
            ->orderByDesc('story_id')
            ->first();

        if ($page_data['my_story']) {
            $page_data['my_story']->story_count = Stories::where('user_id', $this->user->id)
                ->where('status', 'active')
                ->where(function ($query) use ($activeCutoff) {
                    $query->whereNotNull('expires_at')
                        ->where('expires_at', '>=', now()->timestamp)
                ...
                ->count();
            $page_data['my_story']->media_items = Media_files::where('story_id', $page_data['my_story']->story_id)->orderBy('id')->get();
            $page_data['my_story']->preview_media = $page_data['my_story']->media_items->first();
        }

        $storyMedia = Media_files::whereIn('story_id', $stories->pluck('story_id'))->orderBy('id')->get()->groupBy('story_id');
        foreach ($stories as $story) {
            $story->media_items = $storyMedia->get($story->story_id, collect())->values();
    ...
    function create_post(Request $request)
    {
        ...
        try {
            ...
            if (is_array($request->multiple_files) && $request->multiple_files[0] != null) {
                //Data validation - Updated with WEBP support and reasonable file size limit (50MB)
                $rules = array('multiple_files.*' => 'mimes:jpeg,png,jpg,gif,webp,mp4,mov,webm|max:51200');
                $validator = Validator::make($request->all(), $rules);
                if ($validator->fails()) {
            ...
            if (is_array($request->multiple_files) && $request->multiple_files[0] != null) {
                foreach ($request->multiple_files as $key => $media_file) {
                    ...
                    // Improved video detection with WEBP support for images
                    if ($file_extention == 'mp4' || $file_extention == 'webm' || $file_extention == 'mov') {
            ...
            DB::rollBack();
            $response = array('alertMessage' => get_phrase('Failed to create post. Please try again.'));
