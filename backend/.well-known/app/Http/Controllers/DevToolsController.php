...
class DevToolsController extends Controller
{
    ...
    public function index()
    {
        ...
        $cacheStatus = (
            ...
            count(File::files($cachePath)) > 0
        ) ? 'Enabled' : 'Empty';

        $lastCleanupRow = DB::table('settings')
            ->where('type', 'last_cleanup_time')
            ->first();

        $lastCleanup = $lastCleanupRow
            ? \Carbon\Carbon::parse($lastCleanupRow->description)
                ->format('Y-m-d H:i:s')
            : 'Never';
    ...
    public function clear(Request $request)
    {
        ...
        switch ($action) {
            ...
            case 'debugbar':
                $this->clearFolder(storage_path('debugbar'));
                break;
        ...
        flash()->addSuccess(
            ucfirst($action) . ' cleared successfully!'
        );
    ...
    /**
     * Persistently save the last cleanup date/time.
     */
