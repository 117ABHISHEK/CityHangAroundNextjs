    ...
    public function showPlans()
{
    ...
    // Get user's subscriptions for "My Subscriptions" tab
    $mySubscriptions = UserSubscription::with(['subscription.features'])
        ->where('user_id', $user->id)
        ->orderBy('created_at', 'desc')
        ->get();

    // Get payment transactions for each subscription
    $subscriptionData = [];
    foreach ($mySubscriptions as $userSub) {
        // Get payment transaction details
        $paymentTransaction = DB::table('payment_transactions')
            ->where('user_subscription_id', $userSub->subscription_id)
            ->where('user_id', $user->id)
            ->first();
        
        // Get wallet transaction details if payment transaction doesn't exist
        ...
        $subscriptionData[] = [
            ...
            'is_active' => now()->lessThan($userSub->expires_at),
            'days_remaining' => now()->lessThan($userSub->expires_at) ? 
                now()->diffInDays($userSub->expires_at, false) : 0
        ];
    }
    ...
    $page_data['view_path'] = 'subscriptions';
