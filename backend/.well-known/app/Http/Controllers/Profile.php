    ...
    public function profile()
{
    ...
    // 1. User's own + tagged posts
    $userPosts = Posts::where(function ($query) use ($userId) {
    ...
    $followedPageIds = Page_like::where('user_id', $userId)->pluck('page_id')->toArray();
    $pagePosts = Posts::with(['page.categories' => function ($q) {
            $q->orderByDesc('page_category.id');
        }, 'page.city', 'page.area'])
        ->where('posts.publisher', 'page')
        ->whereIn('posts.publisher_id', $followedPageIds)
        ->where('posts.status', 'active')
        ->orderByDesc('posts.post_id')
        ->take(10)
        ->get();

    // 3. Group posts where user is a member
    $groupIds = \App\Models\Group_member::where('user_id', $userId)->pluck('group_id')->toArray(); // assuming group membership model
    $groupPosts = Posts::where('posts.privacy', '!=', 'private')
        ->where('posts.publisher', 'group')
        ->whereIn('posts.publisher_id', $groupIds)
        ->where('posts.status', 'active')
        ->join('users', 'posts.user_id', '=', 'users.id')
        ->select('posts.*', 'users.name', 'users.photo', 'users.friends', 'posts.created_at as created_at')
        ->orderBy('posts.post_id', 'DESC')
        ...
        // 3. Event posts for this user (publisher = 'event')

        $eventIds = Event::where(function($q) use ($userId) {
        $q->where('user_id', $userId)
          ->orWhereJsonContains('going_users_id', $userId)
          ->orWhereJsonContains('interested_users_id', $userId);
    })
    ->pluck('id')
    ->toArray();
   $eventPosts = Posts::where(function ($query) {
        if (auth()->check()) {
