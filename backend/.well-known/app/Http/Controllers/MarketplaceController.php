...
class MarketplaceController extends Controller
{
    private function getMarketFilterData(Request $request, $category_id = null)
    {
        ...
        $filter_area     = $request->input('area_filter', '0') ?: $request->input('area', '0');
        $filter_sort_by  = $request->input('filter_sort_by', 'newest');
        $filter_category = $category_id ?: ($request->input('category_filter', '0') ?: $request->input('category', '0'));
    ...
    public function getCities(Request $request)
    {
        ...
        $query = DB::table('content_master')
            ->join('cities', 'content_master.city_id', '=', 'cities.id')
            ->leftJoin('areas', 'content_master.area_id', '=', 'areas.id')
            ->where(function($q) use ($search) {
                if (!empty($search)) {
                    $q->where('cities.city_name', 'like', "%{$search}%")
                      ->orWhere('areas.area_name', 'like', "%{$search}%");
                }
            })
        ...
        $unique_results = collect($results)->unique('id')->values()->all();
    ...
    public function getProducts(Request $request)
    {
        ...
        $query = DB::table("content_master")
            ...
            ->whereNotNull("title")
            ->where("title", "!=", "")
            ->select("source_id as id", DB::raw("TRIM(title) as text"));
        ...
        if (!empty($location)) {
            ...
            if (!empty($parts[0])) {
                $query->where("city_id", $parts[0]);
            }
            if (!empty($parts[1]) && $parts[1] != '0') {
                $query->where("area_id", $parts[1]);
            }
        }
    ...
    public function allproducts(Request $request)
    {
        ...
        $products_query = Marketplace::with([
            ...
            ->whereHas("page") // ✅ ensures page is not null
            ->withAvg([
                                ...
                                $subQ
                                    ...
                                    ->where(function ($sq) use ($city, $area) {
                                        $sq->whereNull("area_durations")
                                            ->orWhereRaw(
                                                "JSON_SEARCH(JSON_EXTRACT(area_durations, '$.marketplace'), 'one', ?, NULL, '$.*.city') IS NOT NULL",
                                                [$city->id]
                                            )
                                            ->orWhereRaw(
                                                "JSON_SEARCH(JSON_EXTRACT(area_durations, '$.marketplace'), 'one', ?, NULL, '$.*.area') IS NOT NULL",
                                                [$area->id]
                                            );
                                    });
                            });
    ...
    public function productcity(Request $request, $city_slug)
    {
        ...
        if (!empty($filter_category) && $filter_category != "0") {
            $paid_items_query->whereHas("productCategories", function ($q) use ($filter_category) {
                $q->where("product_category_id", $filter_category);
            });
        }
    ...
    public function getSuggestions(Request $request)
    {
        ...
        $products = Marketplace::where("title", "LIKE", "%$query%")->get([
            "id",
            "title",
        ]);
        return response()->json($products);
    ...
    public function store(Request $request)
    {
        ...
        foreach ($multiSelectArray as $cat_id) {
            DB::table("category_product")->insert(["product_category_id" => $cat_id, "product_id" => $product_id]);
        }

        if (DB::table("marketplaces")->where("product_slug", str_slug($product_slug))->count() > 1) {
            $marketplace->update(["product_slug" => str_slug($product_slug) . "-" . $product_id]);
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        $marketplace->update([
            ...
            "title" => $request->title,
            "product_slug" => str_slug($product_slug),
            "brand" => $request->brand,
        ...
        foreach ($multiSelectArray as $cat_id) {
            if (DB::table("category_product")->where("product_category_id", $cat_id)->where("product_id", $id)->count() == 0) {
                DB::table("category_product")->insert(["product_category_id" => $cat_id, "product_id" => $id]);
            }
        }
    ...
    public function single_product($city_slug, $area_slug, $category_slug, $item_slug, $product_category_slug, $product_slug)
    {
        ...
        if (auth()->user() && !in_array(auth()->user()->id, $product_view_data)) {
            ...
            $product->update(["view" => json_encode($product_view_data)]);
        }
        ...
        $page_data["related_product"] = Marketplace::where("product_status", 2)->whereHas("page")->where(fn($q) => $q->where("brand", $product->brand)->orWhere("category", $product->category))->with(["page.city", "page.area", "page.categories", "productCategories"])->get();
