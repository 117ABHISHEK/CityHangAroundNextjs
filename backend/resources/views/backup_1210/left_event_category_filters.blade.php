...
<!-- <div class="row rightSideBarToggler d-hidden">
    ...
    <h2>Filter by</h2>
    <div class="row">
    <form method="GET" action="{{ route('event.category', ['category_slug' => $category->category_slug]) }}">
    ...
    @foreach ($all_event_cities as $key => $city)
    ...
    <div class="tags_left">
        ...
        @foreach ($all_categories as $key => $category)
            <li ><a href="{{ route('event.category',['category_slug'=>$category->category_slug]) }}"> {{ $category->category_name }}</a></li>
         @endforeach   
        </ul>
    </div>
    <a class="btn btn-sm btn-outline-primary rounded" href="{{ route('event.category', ['category_slug' => $category->category_slug]) }}">
        Reset
    </a>
...
<script>
    ...
    @if($filter_city)
            ...
            //(filter_city_id);
             ajax_url_initial_areas = '/ajax/eventareas/'+filter_city_id;
...
if(this.value > 0)
{
    var ajax_url = '/ajax/eventareas/' + this.value;
