...
<div class="group-content profile-content">
    <div class="row gx-3">
        <div class="col-lg-7 col-sm-12"> peobole
            ...
            <div class="people-wrap p-3 border bg-white">
                <div class="member-entry gr-search">
                    ...
                    @endforeach
                    <a href="{{ route('all.people.group.view',$group->id) }}" class="btn d-block mt-4 btn-primary">{{ get_phrase('See More') }}</a>
                </div> <!-- Member Entry End -->
                <div class="group-friend mt-4">
                    <h3 class="h6 fw-7 mb-4">{{ get_phrase('Following') }} {{ $friends_count }}</h3>
                    <div class="gr-wrap mt-3">
                        ...
                        @endif
                            <div class="single-friend gap-3 d-flex align-items-start">
                                ...
                                <a href="{{ route('chat',$friend->getFriend->id) }}" class="btn btn-primary ms-auto"><img src="{{ asset('assets/frontend/images/message-b.png') }}" alt="message">Message</a>
                            </div> 
                        @endforeach
                    </div>
                    ...
                    <div class="gr-wrap mt-3">
                        @foreach ($users as $user)
                            <div class="single-friend gap-3 d-flex align-items-start">
                                ...
                                @else   
                                    <a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('user.friend',$user->id); ?>')" class="btn btn-primary ms-auto"><i class="fa-solid fa-plus"></i> {{ get_phrase('Follow') }} </a>
                                @endif
                            </div> <!-- Follow End -->
                        @endforeach
                    </div>
