...
<div class="col-12">
    ...
    <div class="tags_left">
        ...
        @foreach ($all_categories as $key => $category)
            <li ><a href="{{ route('product.category.city.area',['city_slug'=>$city->city_slug,'category_slug'=>$category->product_category_slug,'area_slug'=>$area->area_slug]) }}"> {{ $category->product_category_name }} in {{$area->area_name}}</a></li>
         @endforeach   
        </ul>
    </div>
    <a class="btn btn-sm btn-outline-primary rounded" href="{{ route('product.category.city.area',['city_slug'=>$city->city_slug,'category_slug'=>$category->product_category_slug,'area_slug'=>$area->area_slug]) }}">
        Reset
    </a>
