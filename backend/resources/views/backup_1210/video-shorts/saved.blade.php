<div class="page-wrap">
    <div class="d-flex pagetab-head  border align-items-center justify-content-between mb-3 py-2 px-3 rounded bg-white">
        ...
        <div class="w-80">
            <a href="javascript:void(0)" onclick="showCustomModal('{{route('load_modal_content', ['view_path' => 'frontend.video-shorts.create'])}}', '{{get_phrase('Create Video & Shorts ')}}');" class="btn btn-primary mt-1" data-bs-toggle="modal"
                data-bs-target="" class="btn btn-primary"> <i
                class="fa-solid fa-clapperboard me-2"></i>{{get_phrase('Create')}}</a>
            <a href="{{ route('videos') }}" class="btn btn-primary mt-1"><i class="fa-solid fa-clapperboard me-2"></i>{{get_phrase('Videos')}}</a>
            <a href="{{ route('shorts') }}" class="btn btn-primary mt-1"><i class="fa-solid fa-clapperboard me-2"></i>{{get_phrase('Shorts')}}</a>
            <a href="{{ route('save.all.view') }}" class="mt-1" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Saved Video"><span><i class="fa-solid fa-bookmark"></i></span></a>
        </div>
    </div>
