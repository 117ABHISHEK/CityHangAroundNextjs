...
<div class="page-wrap">
    <div class="d-flex pagetab-head  border align-items-center justify-content-between mb-3 py-2 px-3 rounded bg-white">
        <h3 class="h5 pt-3"><span><img width="12" src="{{ asset('assets/frontend/images/stickies-fill.png') }}" alt=""></span> {{ get_phrase('My Blog') }}</h3>
        <div class="">
            <a href="{{ route('create.blog') }}" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle me-1"></i>{{ get_phrase('Create Blog') }}</a>
        </div>
    </div>
    ...
    ?>
            <div class="col-12 col-sm-6 col-md-4 col-lg-6 col-xl-4" id="blog-{{ $blog->id }}">
                <article class="single-entry">
                    <div class="entry-img">
                        <a href="{{ route('single.blog',['city_slug'=>$blog->city_slug,'area_slug'=>$blog->area_slug,'category_slug'=>$catslug,'blog_slug'=>$blog->blog_slug]) }}"><img src="{{ get_blog_image($blog->thumbnail,'thumbnail') }}" alt="" class="img-fluid w-100"></a>
