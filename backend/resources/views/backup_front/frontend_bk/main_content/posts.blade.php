...
@foreach($posts as $loopIndex => $post)
            ...
            <div class="entry-header d-flex justify-content-between">
                <div class="ava-info d-flex align-items-center">
                    ...
                    <div class="ava-desc ms-2">
                        <h3 class="mb-0">
                            @if (isset($type)&&$type=="page")
                                <a class="text-black" href="{{route('single.page',['city_slug'=>$post->city_slug,'area_slug'=>$post->area_slug,'category_slug'=>$post->category_slug,'item_slug'=>$post->item_slug])}}">{{$post->title}}</a>
                            @elseif (isset($type)&&$type=="group")
    ...
    if ($isPage) {
        ...
        $profileLink = route('single.page', [
            'city_slug' => $post->page->city->city_slug ?? 'city',
            'area_slug' => $post->page->area->area_slug ?? 'area',
            'category_slug' => $categorySlug,
            'item_slug' => $post->page->item_slug ?? 'item',
        ]);
    } elseif ($isGroup) {
        $lastCategory = optional($post->group->category)->category_slug ?? 'category';
        $profileLink = route('single.group', [
            'category_slug' => $lastCategory,
            'group_slug' => $post->group->group_slug ?? 'group',
            'city_slug' => $post->group->city->city_slug ?? 'city',
            'area_slug' => $post->group->area->area_slug ?? 'area',
        ]);
    } elseif ($isEvent) {
        $lastCategory = optional($post->event->category)->category_slug ?? 'category';
        $profileLink = route('single.event', [
            'city_slug' => $post->event->city->city_slug ?? 'city',
            'area_slug' => $post->event->area->area_slug ?? 'area',
            'category_slug' => $lastCategory,
            'event_slug' => $post->event->event_slug ?? 'event',
        ]);
    } else {
...
@endphp
                            ...
                            <h3 class="mb-0">
                                <a class="text-black" href="{{ $profileLink }}">{{ $displayName }}</a>
                            </h3>
                                ...
                                <!-- <a class="text-black" href="{{ route('user.profile.view',$post->user_id) }}">{{$post->getUser->name}} -->
                                    ...
                                    @if ($post->user_id != auth()->user()->id)
                                        @php
                                            $follow = \App\Models\Follower::where('user_id',auth()->user()->id)->where('follow_id',$post->user_id)->count();
                                        @endphp
                                        @if ($follow>0)
                        ...
                        <div class="text-quote">
                            @if (\Illuminate\Support\Str::contains($post->description, 'http','https'))
                                @php
                                    ...
                                    $publisher_id = DB::table('posts')->where('posts.post_id', $id)->where('publisher','video_and_shorts')->first();
                                    if($publisher_id){
                                    $files = DB::table('videos')->where('videos.id', $publisher_id->publisher_id)->first();
                                    }
                                    else{
                                        $files=[];
                                    }
                                ...
                                @endphp
                                 ...
                                 <video muted controlsList="nodownload" controls class="plyr-js w-100 rounded video-thumb">
                                                        <source src="{{asset('storage/videos/'.$files->file) }}" type="">
                                 </video>
                                 @else
