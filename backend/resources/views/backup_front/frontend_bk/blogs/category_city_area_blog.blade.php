...
<script type="application/ld+json">
	{
  	...
  	"name": "Top 10 {{$category->category_name}} in {{$area->area_name}}, {{$city->city_name}}",
   	"numberOfItems": 5,
  	"itemListElement": [
    	@foreach ($blogs as $paid_items_key => $item)
        @php
      ...
      @endphp
            @if($paid_items_key <9)
                ...
                "@type": "ListItem",
                    ...
                    "name" : "{{$item->title}}",
            "url": "{{ route('single.blog', ['city_slug'=> $item->city_slug, 'area_slug' => $item->area_slug, 'category_slug'=>$catslug , $item->blog_slug]) }}"
...
Top {{ $category->category_name }} business listings in {{$area->area_name}} - {{$city->city_name}} 
    </h1>
   </div>
