...
<div class="widget page-widget">
    <div class="">
        ...
        @endphp
            ...
            @if ($friend>0)
                @if ($friendAccepted>0)
                    <a href="#" class="btn btn-primary" id="btnGroupDrop1" data-bs-toggle="dropdown" aria-expanded="false"><i class="fa-solid fa-user-group"></i> {{ get_phrase('Following') }} </a>
                    <ul class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                        <li><a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('user.unfriend',$user_data->id); ?>')" class="dropdown-item custom_unfrind_button"> <i class="fa-solid fa-xmark"></i>{{ get_phrase('Unfollow') }}</a></li>
                    </ul>
                ...
                @else
                    <form class="ajaxForm" action="{{route('profile.accept_friend_request')}}" method="post">
                        <a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('user.unfriend',$user_data->id); ?>')" class="btn btn-primary px-3" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ get_phrase('Cancel Follow Request') }}"><i class="fa-solid fa-xmark"></i> {{ get_phrase('Cancel') }}</a>
	            		@CSRF
	            		<input type="hidden" name="user_id" value="{{$user_data->id}}">
	                	<button type="submit" id="friendRequestConfirmBtn{{$user_data->id}}" class="btn btn-primary px-4 no-processing no-uploading" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ get_phrase('Accept Follow Request') }}">{{get_phrase('Accept')}}</button>
	                	<button type="button" id="friendRequestAcceptedBtn{{$user_data->id}}" class="btn btn-primary d-hidden  px-4" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ get_phrase('Accepted') }}">{{get_phrase('Accepted')}}</button>
                    </form>
                @endif
            @else   
                <a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('user.friend',$user_data->id); ?>')" class="btn btn-primary"><i class="fa-solid fa-plus"></i> {{ get_phrase('Follow') }} </a>
            @endif
...
<aside class="sidebar plain-sidebar">
    ...
    <div class="widget" id="my-profile-info">
        ...
        <ul>
            ...
            <li>
                ...
                <span>{{get_phrase('Studied at')}} <strong>{{$user_data->studied_at}}</strong></span>
            </li>
            <li>
    ...
    <div class="widget">
        ...
        <a href="{{ route('user.photos',$user_data->id) }}" class="btn btn-primary mt-3 d-block mx-auto">{{ get_phrase('See More') }}</a>
    </div>
    <!--  Widget End -->
    <div class="widget friend-widget">
        ...
        <a href="{{ route('user.friends',$user_data->id) }}" class="btn btn-primary mt-3 d-block mx-auto">{{ get_phrase('See More') }}</a>

        <a href="javascript:void(0);" 
                onclick="openReportModal({{ Auth()->user()->id }}, '{{ Auth()->user()->name }}')" 
