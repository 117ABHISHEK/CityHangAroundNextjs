...
<!-- <div class="row rightSideBarToggler d-hidden">
    ...
    <h2>Filter by</h2>
    <div class="row">
    <form method="GET" action="{{ route('blog.category.city.area', ['city_slug'=>$city->city_slug,'category_slug'=>$category->category_slug,'area_slug' => $area->area_slug]) }}">
 ...
 <div class="tags_left">
        ...
        @foreach ($categories as $key => $category)
            <li ><a href="{{ route('blog.category.city.area', ['city_slug'=>$city->city_slug,'category_slug'=>$category->category_slug,'area_slug' => $area->area_slug]) }}"> {{ $category->category_name }} in {{$area->area_name}}</a></li>
         @endforeach   
        </ul>
    </div>

    
    <a class="btn btn-sm btn-outline-primary rounded" href="{{ route('blog.category.city.area', ['city_slug'=>$city->city_slug,'category_slug'=>$category->category_slug,'area_slug' => $area->area_slug]) }}">
        Reset
    </a>
