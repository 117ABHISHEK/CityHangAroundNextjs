...
<div class="col-12">
                    <strong>Total Results Found  : {{$mypages->count() }}</strong>
                    Results
                </div>
    <div class="widget">
    ...
    <h2>Filter by</h2>
    <div class="row">
    <form method="GET" action="{{ route('page.city',['city_slug'=>$city->city_slug]) }}">
    ...
    <div class="tags_left">
        ...
        @foreach ($all_categories as $key => $category)
            <li ><a href="{{ route('page.category.city',['category_slug'=>$category->category_slug,'city_slug'=>$city->city_slug]) }}"> {{ $category->category_name }} in {{$city->city_name}}</a></li>
         @endforeach   
        </ul>
    </div>
    <a class="btn btn-sm btn-outline-primary rounded" href="{{ route('page.category.city',['category_slug'=>$category->category_slug,'city_slug'=>$city->city_slug]) }}">
        Reset
    </a>
