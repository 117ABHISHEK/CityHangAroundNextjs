                ...
                @forelse($cityScores as $index => $score)
                    <tr class="border-top">
                        <td class="ps-4 text-muted">{{ $index + 1 }}</td>
                          @if($score['iscategory']=='N')
                        <td class="fw-medium text-dark">
                      
                          <a href="{{ route('admin.city.activity.report', ['cityId' => $score['id'], 'user_id' => $user->id] + request()->query()) }}" class="text-decoration-none fw-semibold text-primary">
