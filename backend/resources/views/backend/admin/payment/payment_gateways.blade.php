        ...
        <div class="row">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center flex-wrap gr-15">
                    ...
                    <a href="{{ route('admin.payment_gateway.create') }}" class="btn btn-primary">
                        + {{ get_phrase('Add New') }}
                    </a>
                </div>
            </div>
                                    ...
                                    <td class="text-center">
                                        <div class="adminTable-action me-auto">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end eDropdown-menu-2 eDropdown-table-action">
                                                ...
                                                @if($payment_gateway->status == 1)
                                                    <li><a class="dropdown-item" onclick="return confirm('{{ get_phrase('Are you sure want to change status?') }}')" href="{{ route('admin.payment_gateway.status', $payment_gateway->id) }}">{{ get_phrase('Deactivate') }}</a></li>
                                                @else
                                                    <li><a class="dropdown-item" onclick="return confirm('{{ get_phrase('Are you sure want to change status?') }}')" href="{{ route('admin.payment_gateway.status', $payment_gateway->id) }}">{{ get_phrase('Activate') }}</a></li>
                                                @endif
                                                @if($payment_gateway->test_mode == 1)
