...
<div class="container mt-4">
    <div class="card shadow-lg">
        <div class="card-header bg-warning text-white">
            <h4 class="mb-0"> <i class="fas fa-edit"></i> Edit State</h4>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.states.update', $state->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="state_name" class="form-control @error('state_name') is-invalid @enderror" 
                           placeholder="Enter State Name" required value="{{ old('state_name', $state->state_name) }}">
                    @error('state_name')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="state_abbr" class="form-control @error('state_abbr') is-invalid @enderror" 
                           placeholder="Enter Abbreviation (e.g., AP)" required value="{{ old('state_abbr', $state->state_abbr) }}">
                    @error('state_abbr')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
