<div class="main_content">
    ...
    <div class="mainSection-title py-3">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        ...
                        <div class="export-btn-area">
                            <a href="{{ route('admin.users') }}" class="btn btn-primary" data-bs-toggle="tooltip"
                                data-bs-placement="top" data-bs-custom-class="custom-tooltip"
                                title="{{ get_phrase('Back') }}">
                                {{ get_phrase('Back') }}
                            </a>
                        </div>
                    </div>
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-12 col-sm-10 col-md-8 col-lg-6">
                <div class="eSection-wrap-2 py-4">
                    <div class="eForm-layouts">
                        <form method="POST" action="{{ route('admin.user.store') }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                <label class="form-label eForm-label d-block">{{ get_phrase('Gender') }}</label>
                                <div class="form-check form-check-inline">
                                    <input type="radio" id="male" name="gender" value="male" class="form-check-input">
                                    <label for="male" class="form-check-label">{{ get_phrase('Male') }}</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input type="radio" id="female" name="gender" value="female" class="form-check-input">
                                    <label for="female" class="form-check-label">{{ get_phrase('Female') }}</label>
                                </div>
                            </div>

                            <div class="mb-3">
                                <label for="photo" class="form-label eForm-label">{{ get_phrase('Photo') }}</label>
                                <input type="file" class="form-control eForm-control" id="photo" name="photo">
                            </div>

                            <div class="mb-3">
                                <label for="date_of_birth"
                                    class="form-label eForm-label">{{ get_phrase('Date of birth') }}</label>
                                <input type="text" class="form-control eForm-control inputDate" id="date_of_birth"
                                    name="date_of_birth" placeholder="{{ get_phrase('Date of birth') }}" required>
                            </div>

                            <div class="mb-3">
                                <label for="bio" class="form-label eForm-label">{{ get_phrase('Bio') }}</label>
                                <textarea name="bio" id="bio" class="form-control eForm-control"
