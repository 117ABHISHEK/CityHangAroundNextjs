    ...
    <div class="page-wrap">
        ...
        <div class="card page-card p-4 mt-4">
            <h3 class="sub-title">{{ get_phrase('Merchants') }}</h3>
                ...
                @foreach ($pages as $key => $mypage )
                    <div class="col-lg-4 col-xl-3 col-md-4 col-6">
                        <div class="card sugg-card p-2 rounded">
                            <a href="{{ route('single.page',['city_slug'=>$mypage->city_slug,'area_slug'=>$mypage->area_slug,'category_slug'=>$mypage->category_slug,'item_slug'=>$mypage->item_slug]) }}" class="mb-2 thumbnail-110-auto" style="background-image: url('{{ get_page_logo($mypage->logo, 'logo', $mypage->categories) }}')"></a>
                            <h4><a href="{{ route('single.page',['city_slug'=>$mypage->city_slug,'area_slug'=>$mypage->area_slug,'category_slug'=>$mypage->category_slug,'item_slug'=>$mypage->item_slug]) }}">{{ ellipsis($mypage->title,10) }}</a></h4>
                            @php
            ...
            @if (count($pages)>4)
                <a href="{{ url('search/page?search='.$_GET['search']) }}" class="btn btn-primary btn-sm mt-3">{{ get_phrase('See More') }}</a>
            @endif
        </div> <!-- Search Card End -->
        <div class="card people-card p-4 mt-4 mb-3">
            ...
            <div class="people-wrap mt-2">
                <div class="people-item d-sm-flex mb-3 justify-content-between w-100">
                    ...
                    @if ($friend>0)
                        @if ($friendAccepted>0)
                            <a href="#" class="btn btn-primary align-self-start"><i class="fa-solid fa-user-group"></i> {{ get_phrase('Following') }} </a>
                        @else
                            <a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('user.unfriend',$people->id); ?>')" data-bs-toggle="tooltip" data-bs-placement="top" title="Cancel Follow Request" class="btn btn-primary align-self-start"><i class="fa-solid fa-xmark"></i> {{ get_phrase('Cancel') }}</a>
                        @endif
                    @else   
                        <a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('user.friend',$people->id); ?>')" class="btn btn-primary align-self-start mt-2"><i class="fa-solid fa-plus"></i> {{ get_phrase('Follow') }} </a>
                    @endif
            ...
            @endforeach
            @if (count($peoples)>4)
                <a href="{{ url('search/people?search='.$_GET['search']) }}" class="btn btn-primary btn-sm mt-3 ">{{ get_phrase('See More') }}</a>
            @endif
        </div> <!-- Follow Card End -->
        <div class="card people-card p-4 mt-4 mb-3">
            ...
            @if (count($posts)>2)
                <a href="{{ url('search/post?search='.$_GET['search']) }}" class="btn btn-primary btn-sm mt-3 ">{{ get_phrase('See More') }}</a>
            @endif
        </div>


        <div class="card p-3 mt-4">
            <h3 class="sub-title">{{ get_phrase('Deals') }}</h3>
            <div class="row">
                @include('frontend.marketplace.product-single',['products'=>$products,'search'=>'search'])
            </div>
            @if (count($products)>3)
                <a href="{{ url('search/product?search='.$_GET['search']) }}" class="btn btn-primary d-block mt-3 btn-sm">{{ get_phrase('See More') }}</a>
            @endif
        ...
        <div class="card video-cards p-4 mt-4">
            ...
            @endforeach
            @if (count($videos)>3)
                <a href="{{ url('search/video?search='.$_GET['search']) }}" class="btn btn-primary d-block mt-3 btn-sm">{{ get_phrase('See More') }}</a>
            @endif
        </div> <!-- Video card End -->
        <div class="card p-4 mt-4">
            <h3 class="sub-title">{{ get_phrase('Community') }}</h3>
                ...
                <div class="col-md-3 col-lg-4 col-xl-3 col-6">
                    <div class="card sugg-card p-2 rounded">
                        <div class="mb-2 thumbnail-103-103" style="background-image: url('{{ get_group_logo($group->logo,'logo') }}');"></div>
                                    <a href="{{ route('single.group.details',$group->id) }}"><h4>{{ ellipsis($group->title,10) }}</h4></a>
            ...
            @if (count($groups)>3)
                <a href="{{ url('search/group?search='.$_GET['search']) }}" class="btn btn-primary btn-sm mt-3">{{ get_phrase('See More') }}</a>
            @endif
        </div><!--  Group Card End -->
        ...
        <div class="event-card p-3 card mt-3">
            ...
            @if (count($events)>3)
                <a href="{{ url('search/event?search='.$_GET['search']) }}" class="btn btn-sm btn-primary d-block">{{ get_phrase('More Events') }}</a>
            @endif
        </div> <!-- Event Cards End -->
