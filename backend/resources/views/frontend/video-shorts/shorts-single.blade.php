                        ...
                        <div class="avatar-body d-flex align-items-center justify-content-between">
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                ...
                                <div class="avatar-info ms-2">
                                    ...
                                    <h6 class="mb-0 "><a href="{{route('user.profile.view', $short->getUser->id)}}" class="custom-text-shadow">{{ $short->getUser->name }} </a></h6>
                                    @php
    $timezone = optional(Auth::user())->timezone ?? config('app.timezone');
@endphp

<span class="small-text">
{{ \Carbon\Carbon::parse($short->created_at)->timezone($timezone)->format("M d") }}
at
{{ \Carbon\Carbon::parse($short->created_at)->timezone($timezone)->format("h:i A") }}
</span>
                                </div>
                            </div>
                            @php
                    ...
                    <div class="video-share fs-4">
                        ...
                        <a href="#" onclick="event.stopPropagation(); showCustomModal('{{route('load_modal_content', ['view_path' => 'frontend.main_content.share_post_modal', 'post_id' => $post->post_id] )}}', '{{get_phrase('Share post')}}');"> <i class="fa-solid fa-share"></i><span class="fs-6 responsive-share-text custom-text-shadow">{{get_phrase('Share')}}</span></a>
...
<!--<--------------------- this media query for shorts, share text ---------------->
<style>
