                    ...
                    @if(count($product_image) > 0)
                        @foreach ($product_image as $image )
                            <div class="carousel-item {{ $loop->index=='0'? "active":"" }}"  onclick="showCustomModal('{{route('load_modal_content', ['view_path' => 'frontend.marketplace.load_image', 'image' => $image->file_name])}}', '');">
                                <img class="rounded w-100" src="{{ get_product_image($image->file_name,"coverphoto") }}" alt="{{ $product->title }}"> 
                            </div>
                        @endforeach
                    @elseif(!empty($product->image))
                        <div class="carousel-item active">
                            <img class="rounded w-100" src="{{ get_marketplace_banner_image($product, 'coverphoto') }}" alt="{{ $product->title }}" onerror="this.onerror=null; this.src='{{ get_product_image('','coverphoto') }}';"> 
                        </div>
                    @else
                        <div class="carousel-item active">
                            <img class="rounded w-100" src="{{ get_product_image('','coverphoto') }}" alt="{{ $product->title }}"> 
                        </div>
                    @endif
                </div>
        ...
        <div class="col-lg-6">
            <div class="product-info">
                ...
                <span style="margin-right: 10px;">
                    {{ is_array(json_decode($product->view, true)) ? count(json_decode($product->view, true)) : 0 }}
                    {{ get_phrase('Views') }}
                </span>
                    </div>
    ...
    <div class="card product p-3">
        <div class="product-figure position-relative">
            <a href="{{ route('single.product', [
                ...
                <div class="thumbnail-90-90" style="background-image: url('{{ get_marketplace_banner_image($product, 'coverphoto') }}');"></div>
            </a>
