...
<form class="ajaxForm" action="{{ route('product.update',$product->id, false) }}" method="POST" enctype="multipart/form-data">
    @csrf
   ...
   <div class="form-group">
    ...
    $ids = $product->category; // your comma-separated string
    $idArray = explode(',', $ids); // convert to array

    $categories = Category::whereIn('id', $idArray)->get();
@endphp

    <!-- <div id="selected-value"></div> -->
    <div class="form-group">
        <label for="#">{{ get_phrase('Product Tags/keyword') }}</label>
    ...
    <div class="form-group row">
        <div class="col-md-12">
            ...
            <select name="brand" id="brand" class="selectpicker form-control @error('category') is-invalid @enderror">
    ...
    <div class="form-group">
        ...
        <select name="currency" id="currency" class="selectpicker form-control @error('category') is-invalid @enderror">
    ...
    <div class="row">
        ...
        <input type="number" name="price" class="border-0 bg-secondary" placeholder="Your Price" value="{{ $product->product_original_price }}">
    </div>
        </div>
        ...
        <input type="number" name="selling_price" class="border-0 bg-secondary" placeholder="Your Selling Price" value="{{ $product->product_selling_price }}">
    </div>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <input type="text" name="video_url" class="border-0 bg-secondary" placeholder="Video URL" value="{{ $product->video_url }}">
    </div>
        </div>
    ...
    <div class="row">
        ...
        <input type="date" name="start_date" class="border-0 bg-secondary" placeholder="Start Date" value="{{ $product->startdate ? date('Y-m-d', strtotime($product->startdate)) : '' }}">
    </div>
        </div>
        ...
        <input type="date" name="end_date" class="border-0 bg-secondary" placeholder="End Date" value="{{ $product->startdate ? date('Y-m-d', strtotime($product->enddate)) : '' }}">
    </div>
        </div>
    </div>
    <div class="form-group" hidden>
     ...
     <div class="form-group">
        ...
        <input type="url" name="buy_link" id="buy_link" class="border-0 bg-secondary" placeholder="{{get_phrase('Enter the enquire link')}}" value="{{ $product->buy_link }}">
    </div>
    ...
    <p>Featured Video</p>
      <div class="mb-3">
          ...
          <input type="file" name="featured_video" class="border-0 bg-secondary" id="featured_video" class="form-control">
      </div>
...
<script>
 ...
 $(document).ready(function() {
    // Initialize standard selectpickers that don't have custom initializations
    $('#parent, #brand, #currency').each(function() {
        $(this).select2({
            dropdownParent: $(this).parent()
        });
    });

    // Initialize category parent dropdown inside category modal
...
var ajax_url = "{{route('ajax.storeproductcategories.parent', [], false)}}";
    ...
    if(result > 0)
    {
      // console.log(result);
        ...
        var ajax_url = "{{route('page.json.parent.product.catgories', [], false)}}";
...
var ajax_url = "{{route('ajax.storeproductcategories', [], false)}}";
    ...
    if(result > 0)
    {
      // console.log(result);
        ...
        // $('#category').html("<option selected value='0'>{{ __('prefer_country.loading-wait') }}</option>");
        //$('#category').selectpicker('refresh');
        var ajax_url = "{{route('page.json.product.catgories', [], false)}}";
...
var ajax_url = "{{route('ajax.store.brand', [], false)}}";
    ...
    if(result > 0)
    {
      // console.log(result);
        ...
        // $('#category').html("<option selected value='0'>{{ __('prefer_country.loading-wait') }}</option>");
        //$('#category').selectpicker('refresh');
        var ajax_url = "{{route('product.json.brand', [], false)}}";
