...
<div class="widget_top_filter p-3 border rounded bg-white">
    <!-- Results Count -->
    <div class="mb-3">
        <strong>Total Results Found: {{ $total_results ?? 0 }}</strong> Results
    </div>
    ...
    <form method="GET" action="{{ route('category.group', ['category_slug' => $category->category_slug]) }}">
        ...
        <div class="mb-3">
            ...
            <select id="filter_sort_by" name="filter_sort_by" class="form-select">
        ...
        <div class="d-flex gap-2 mb-3">
            <a class="btn btn-outline-secondary w-50" href="{{ route('category.group', ['category_slug' => $category->category_slug]) }}">
                Reset
            </a>
    ...
    <!-- Category Tags Carousel -->
      <div class="card blog-tags p-4" style="padding: 10px !important;">
        <div class="tags ">
            
             @foreach ($all_categories as $category)
              
                    <a href="{{ route('category.group',['category_slug'=>$category->category_slug]) }}"  style="font-size: 13px;padding: 4px 6px;" >
    ...
    <!--        @foreach ($all_categories as $category)-->
    <!--            <div class="item">-->
    <!--                <a href="{{ route('category.group',['category_slug'=>$category->category_slug]) }}" class="badge bg-light text-dark p-2">-->
    <!--                    {{ $category->category_name }}-->
    <!--                </a>-->
    <!--            </div>-->
    <!--        @endforeach-->
...
<script>
  ...
  $('.selector2').select2({
    dropdownParent: $('.widget_top_filter'),
    width: '100%'
  });
