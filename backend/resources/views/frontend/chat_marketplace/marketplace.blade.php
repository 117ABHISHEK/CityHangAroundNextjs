...
<div class="chat-wrapper">
    <div class="chat-title">
        Chat about {{ $conversation->marketplace->title ?? 'Product' }}
    ...
    <form id="chat-form" class="chat-form" enctype="multipart/form-data">
        ...
        <input type="text" name="message" id="message-input"
               ...
               value="{{ $hasUserChatted ? '' : 'Hi, I am interested in your Product.' }}" />
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const defaultMessage = "Hi, I am interested in your listing.";
    ...
    function sendMessage(message = '', imageFile = null) {
        ...
        fetch("{{ route('chat.marketplace.send') }}", {
            method: "POST",
            headers: {
    ...
    function fetchNewMessages() {
        fetch(`{{ route('chat.marketplace.fetch', ['id' => $conversation->id]) }}?last_id=${lastMessageId}&_=${Date.now()}`)
            .then(res => res.json())
            .then(data => {
