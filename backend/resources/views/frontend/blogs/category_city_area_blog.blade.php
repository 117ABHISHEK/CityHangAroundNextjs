...
foreach ($blogs->take(10) as $index => $item) {

    $catslug = DB::table('blogcategories')
        ->join('blog_category','blogcategories.id','=','blog_category.category_id')
        ->where('blog_category.blog_id', $item->id)
        ->value('category_slug');
