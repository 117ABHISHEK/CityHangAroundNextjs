...
<div class="widget_top_filter d-none d-lg-block">
    ...
    <form method="GET" action="{{ route('blog.city.area', ['city_slug'=>$city->city_slug,'area_slug' => $area->area_slug]) }}" id="filterFormDesktop">
        <div class="left_size_custom_section">
            ...
            <div class="form-group">
                ...
                <select id="category_filter" name="category" class="custom-select-premium">
                    ...
                    @foreach ($categories as $cat)
                        <option value="{{ route('blog.city.area', ['city_slug'=>$city->city_slug,'area_slug' => $area->area_slug]) }}">
                            {{ $cat->category_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="action-row">
                <a class="btn-reset" href="{{ route('pages') }}">
                    Reset
                </a>
        ...
        <ul class="category-list">
            @foreach ($categories as $key => $cat)
                <li class="category-item {{ $key >= 10 ? 'hidden-category' : '' }}">
                    <a href="{{ route('blog.city.area', ['city_slug'=>$city->city_slug,'area_slug' => $area->area_slug]) }}">
                        {{ $cat->category_name }}
                    </a>
                </li>
            @endforeach
...
<div id="mobileSidebar" class="d-lg-none">
    ...
    <div class="p-4">
        ...
        <form method="GET" action="{{ route('blog.city.area', ['city_slug'=>$city->city_slug,'area_slug' => $area->area_slug]) }}" id="filterForm">
            <div class="left_size_custom_section">
                <!-- Mobile City Search -->
                <div class="form-group">
        ...
        <div class="category-box mt-4">
            ...
            <ul class="category-list">
                @foreach ($categories as $key => $cat)
                    <li class="category-item {{ $key >= 10 ? 'hidden-category' : '' }}">
                        <a href="{{ route('category.blog',['category_slug'=>$cat->category_slug]) }}">
                            {{ $cat->category_name }}
                        </a>
                    </li>
                @endforeach
