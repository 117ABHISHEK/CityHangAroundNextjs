<form class="ajaxForm" action="{{ route('add.image.album') }}" method="post" enctype="multipart/form-data">
  ...
  <div class="mb-3 w-100 d-flex">
    ...
    $page =DB::table('pages')->select('pages.id','pages.item_slug','pages.logo','pages.title','cities.city_slug','areas.area_slug','pagecategories.category_slug')
        ...
        ->join('page_likes','page_likes.page_id','pages.id')
    ...
    @endphp
      <a href="{{route('single.page',['city_slug'=>$page[0]->city_slug,'area_slug'=>$page[0]->area_slug,'category_slug'=>$page[0]->category_slug,'item_slug'=>$page[0]->item_slug])}}" class="author-thumb d-flex align-items-center">
        <img src="{{get_page_logo($page[0]->logo, 'logo')}}" class="rounded-circle user_image_show_on_modal" alt="">
        <h6 class="ms-2">{{$page[0]->title}}</h6>
      </a>
    @elseif (isset($group_id)&&!empty($group_id))
      @php
        $group = \App\Models\Group::find($group_id);
      @endphp
        <a href="{{route('single.group.details',$group_id)}}" class="author-thumb d-flex align-items-center">
