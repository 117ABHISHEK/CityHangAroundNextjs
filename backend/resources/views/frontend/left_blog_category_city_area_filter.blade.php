...
<div class="widget_top_filter d-none d-lg-block">
    <div class="mb-4 d-flex justify-content-between align-items-center">
        <span class="text-secondary small fw-bold">Total Results</span>
        <span class="badge bg-light text-dark border px-2.5 py-1.5 rounded-pill">{{$blogs->total()}}</span>
    </div>

    <form method="GET" action="{{ route('blog.category.city.area', ['city_slug'=>$city->city_slug,'category_slug'=>$category->category_slug,'area_slug' => $area->area_slug]) }}" id="filterFormDesktop">
        <div class="left_size_custom_section">
            ...
            <div class="form-group">
                ...
                <select id="category_filter" name="category" class="custom-select-premium">
                    ...
                    @foreach ($categories as $cat)
                        <option value="{{ route('blog.category.city.area', ['city_slug'=>$city->city_slug,'category_slug'=>$cat->category_slug,'area_slug' => $area->area_slug]) }}" {{ $category->id == $cat->id ? 'selected' : '' }}>
                            {{ $cat->category_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="action-row">
                <a class="btn-reset" href="{{ route('blog.category.city.area', ['city_slug'=>$city->city_slug,'category_slug'=>$category->category_slug,'area_slug' => $area->area_slug]) }}">
                    Reset
                </a>
        ...
        <ul class="category-list">
            @foreach ($categories as $key => $cat)
                ...
                <li class="category-item {{ $key >= 10 ? 'hidden-category' : '' }} {{ $isActive ? 'active' : '' }}">
                    <a href="{{ route('blog.category.city.area', ['city_slug'=>$city->city_slug,'category_slug'=>$cat->category_slug,'area_slug' => $area->area_slug]) }}" class="{{ $isActive ? 'active' : '' }}">
                        {{ $cat->category_name }}
                    </a>
                </li>
            @endforeach
...
<div id="mobileSidebar" class="d-lg-none">
    ...
    <div class="p-4">
        ...
        <form method="GET" action="{{ route('blog.category.city.area', ['city_slug'=>$city->city_slug,'category_slug'=>$category->category_slug,'area_slug' => $area->area_slug]) }}" id="filterForm">
            <div class="left_size_custom_section">
                <!-- Mobile City Search -->
                <div class="form-group">
...
// Sidebar Toggles using robust event delegation and direct binding
    ...
    // Category Selector redirect
    $('#category_filter, #mcategory_filter').on('change', function() {
        var url = $(this).val();
        if(url) {
