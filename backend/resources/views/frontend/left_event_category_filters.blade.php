...
<style>
  ...
  padding-right: 0 !important;
    ...
    .category-item a {
        ...
        border-radius: 50px;
        margin-bottom: 2px;
        box-shadow: 0 4px 6px rgba(0,0,0,0.08); /* Modern Shadow */
        transition: all 0.2s ease;
    }
...
<div class="widget_top_filter hidden lg:block">
    <div class="mb-2">
        <strong>Total Results Found: {{$events->total()}}</strong>
    </div>

    <form method="GET" action="{{ route('event.category', ['category_slug' => $category->category_slug]) }}"  id="filterFormDesktop">
            ...
            <div class="form-group mb-1">
                ...
                <select id="category_filter" name="category" class="selectpicker form-control" data-live-search="true"  data-width="100%" title="Select Category">
                   ...
                   @foreach ($all_categories as $key => $category)
                                        ...
                                        <option value="{{ route('event.category',['category_slug'=>$category->category_slug]) }}">
        ...
        title="Sort_By">
            ...
            <div class="flex gap-2 mt-4">
                ...
                <a class="btn btn-outline-primary rounded w-1/2 flex items-center justify-center text-sm" href="{{ route('event.category', ['category_slug' => $category->category_slug]) }}">
                    Reset
                </a>
            </div>
        </div>
        ...
        <ul class="category-list">
           ...
           @foreach ($all_categories as $key => $category)
            <li class="category-item {{ $key >= 10 ? 'hidden-category' : '' }}">
               <a href="{{ route('event.category',['category_slug'=>$category->category_slug]) }}">
                     {{ $category->category_name }}
                </a>
...
<div id="mobileSidebar" class="fixed top-0 left-0 h-full w-72 bg-white shadow-lg transform -translate-x-full transition-transform duration-300 z-50 overflow-y-auto lg:hidden">
    ...
    <div class="p-4">
        ...
        <form method="GET" action="{{ route('event.category', ['category_slug' => $category->category_slug]) }}" id="filterFormMobile">
            <div class="left_size_custom_section">
                <!-- Mobile City Search -->
                ...
                <div class="form-group mb-3">
                    <!-- <label class="text-xs font-bold text-gray-500 uppercase">Category</label> -->
                    <select id="mcategory_filter" name="category" class="selectpicker form-control" data-live-search="true"  data-width="100%" title="Select Category">
                        <option value="">Select Category</option>
