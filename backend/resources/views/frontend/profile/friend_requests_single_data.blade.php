@foreach($friend_requests as $friend_request)
	...
	<div class="single-item-countable col-6" id="friendRequest{{$requester_user_data->id}}">
	    <div class="card">
	        <div class="mb-2"><img src="{{get_user_image($requester_user_data->photo, 'optimized')}}" class="rounded img-fluid" alt=""></div>
	        <div class="card-details">
	            <h6><a href="{{route('user.profile.view', $requester_user_data->id)}}">{{$requester_user_data->name}}</a></h6>
	            <span class="mute">{{$number_of_mutual_friends}} {{get_phrase('Mutual Following')}}</span>
