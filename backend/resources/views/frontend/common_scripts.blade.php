<script type="text/javascript">
		...
		$.ajax({
			...
			success: function(response){
				if(response){
					...
					// Automatically trigger the first story if it exists
					if($('.story-entry-details.active').length > 0){
						$('.story-entry-details.active').trigger('click');
					}
				}
	...
	function distributeServerResponse(response){
		if (response) {
			// Safely parse: only parse strings, accept already-parsed objects too
			if (typeof response === 'string') {
				try { response = (typeof response === 'string' ? JSON.parse(response) : response); } catch(e) { console.error('distributeServerResponse parse error', e); return; }
			}
		    ...
		    if(typeof response.prependElement != "undefined" && response.prependElement != 0){
                ...
                if ($newContent.length > 0) {
                    var $entry = $newContent.hasClass('single-entry') ? $newContent : $newContent.find('.single-entry');
                    if ($entry.length > 0 && $entry.attr('id')) {
                        var targetId = $entry.attr('id');
                        // Search inside the target container (e.g. #timeline-posts) for the post ID to prevent duplicates
                        if ($(response.prependElement).find('#' + targetId).length > 0 || $('#' + targetId).length > 0) {
                            isDuplicate = true;
                        }
                    }
                }
                ...
                if (!isDuplicate) {
                    ...
                    console.warn("Blocked duplicate prepend for post element");
                }
		    }
...
function myMessageReact(react, requestType, messageId){
	...
	// Auto-initialize Select2 for City/Area filter dropdowns to enable search/typing
	function initFilterDropdownSelect2() {
		if (window.jQuery && jQuery.fn.select2) {
			$('#city, #mcity, #area, #marea').each(function () {
				const $field = $(this);

				// Bootstrap selectpicker manages these dropdowns on most filter sidebars.
				// Initializing Select2 on the same element causes flaky rendering and stale state.
				if ($field.hasClass('selectpicker') || $field.hasClass('select2-hidden-accessible')) {
					return;
				}

				$field.select2({
					placeholder: $field.is('#city, #mcity') ? 'Select City' : 'Select Area',
					allowClear: true,
					width: '100%'
				});
			});
		} else {
			setTimeout(initFilterDropdownSelect2, 100);
		}
	}
	$(document).ready(function() {
		initFilterDropdownSelect2();
	});

	// Select2-only helper. Kept off the global loadAreas name so page-specific
	// Bootstrap-select filters can keep their own implementations without collisions.
	function loadAreasSelect2(cityId, areaSelector) {
		...
		$.ajax({
			url: "<?php echo url('ajax/blogareas'); ?>/" + cityId,
			type: "GET",
			success: function (response) {
