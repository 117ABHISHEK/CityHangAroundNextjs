<div class="all-notify-control header-controls bg-white shadow-sm">
    ...
    ?>
                        ...
                        @elseif ($newNotification->type=="marketplace_enquiry")
                        <li class="notify-item friendr-item">
                            <div class="avatar-details">
                                <div class="d-flex">
                                    <div class="noti-avata me-3"><img width="48" class="rounded-circle" src="{{ get_user_image($newNotification->getUserData->id,"optimized") }}" alt="">
                                        <span class="notify-type"><i class="fa fa-shopping-bag"></i></span>
                                    </div>
                                    <div class="noti-details">
                                        <h4>
                                            <a href="{{ route('user.profile.view',$newNotification->getUserData->id) }}">{{ ucfirst($newNotification->getUserData->name) }}</a>
                                            <span>submitted an enquiry for product/service in your location.</span>
                                        </h4>
                                        <div class="btn-inline" style="gap: 5px;">
                                            <a href="{{ route('admin.lead.purchase.report') }}" class="btn btn-primary btn-sm">{{ get_phrase('View Leads') }}</a>
                                            @if ($newNotification->status=='0')
                                                <a href="javascript:void(0)" onclick="ajaxAction('<?php echo route('mark.as.read.notification',$newNotification->id); ?>')" class="btn btn-secondary btn-sm">{{ get_phrase('Mark As Read') }}</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
