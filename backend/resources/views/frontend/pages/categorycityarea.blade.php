...
foreach ($mypages->take(10) as $index => $item) {
    $primaryCategory = $item->categories->last();
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb bg-white pl-0 pr-0">
                                ...
                                @endforeach
                                 ...
                                 <li class="breadcrumb-item"><a href="{{ route('page.city.area', ['city_slug' => $city->city_slug,'area_slug' => $area->area_slug]) }}">{{ $area->area_name }}</a></li>           
...
<h1 class="font-weight-light text-primary mb-3">
Explore the Best {{ $category->category_name }} Services in {{$area->area_name}}, {{$city->city_name}}!
    </h1>

    @foreach ($mypages as $key => $mypage)
    @php
        $pagecategory = $mypage->categories->last(); // or first() if preferred
        $catslug = $pagecategory?->category_slug ?? 'general';
            ...
            @endif
             <br>
                ...
                <a href="{{ route('single.page', [
                    ...
                    <span><i class="fa fa-thumbs-up"></i> {{ $mypage->likes_count ?? 0 }} {{ get_phrase('People like this') }}</span>
                </a>
            </div>
        </div>
    </div>
...
<div class="mt-6 bg-white shadow-lg rounded-lg p-6 hidden" style="padding:20px;" >
    <!-- Section Title -->
