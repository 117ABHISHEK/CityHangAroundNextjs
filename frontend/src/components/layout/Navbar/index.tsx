    ...
    <div className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="container mx-auto flex flex-col gap-4 px-6 py-4 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-4">
          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-red-600 text-lg font-bold text-white">C</div>
          <div>
        ...
        <div className="flex flex-1 flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-2 shadow-sm md:max-w-2xl">
            <span className="rounded-2xl bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm">📂 All Categories</span>
            <div className="flex flex-1 items-center gap-2 px-3 text-sm text-slate-500">
              <span>🔎</span>
              <input type="text" placeholder="Search for restaurants, services, events..." className="w-full bg-transparent outline-none" />
            </div>
            <button className="rounded-2xl bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700">Search</button>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-3">
            ...
            <button className="rounded-full bg-red-600 px-5 py-2 text-sm font-semibold text-white hover:bg-red-700">+ Add Business</button>
          </div>
        </div>
      </div>
      <nav className="border-t border-slate-200 bg-white shadow-sm">
        <div className="container mx-auto flex items-center gap-4 overflow-x-auto px-6 py-3 text-sm text-slate-600">
          {['Home', 'City Guide', 'Buy/Sell', 'Marketplace', 'Community', 'Blog', 'Contact'].map((label, index) => (
            <a key={label} href="#" className={`flex items-center gap-2 rounded-full px-3 py-2 hover:bg-slate-100 ${index === 0 ? 'font-semibold text-red-600' : ''}`}>
